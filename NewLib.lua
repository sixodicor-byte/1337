...
do
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        if Info.ColorPicker then
            local CpInfo = Info.ColorPicker
            local CpIdx = CpInfo.Idx or (Idx .. '_Color')
            CpInfo.Idx = nil
...
function ThemeManager:ThemeUpdate()
    for _, field in ipairs({'FontColor', 'MainColor', 'AccentColor', 'BackgroundColor', 'OutlineColor'}) do
        if Options[field] then self.Library[field] = Options[field].Value end
    end
...
function ThemeManager:ApplyTheme(name)
    ...
    local colors = custom or entry[2]
    for field, value in pairs(colors) do
        local color = Color3.fromHex(value)
        self.Library[field] = color
        if Options[field] then Options[field]:SetValueRGB(color) end
    end
    self:ThemeUpdate()
end
function ThemeManager:SaveDefault(name)
    if type(name) == 'string' and name ~= '' then writefile(self.Folder .. '/themes/default.txt', name) end
end
function ThemeManager:LoadDefault()
    ...
    if isfile(path) then
        ...
        if self.BuiltInThemes[saved] or self:GetCustomTheme(saved) then name = saved end
    end
    if Options.ThemeManager_ThemeList then Options.ThemeManager_ThemeList:SetValue(name) else self:ApplyTheme(name) end
end
function ThemeManager:SaveCustomTheme(name)
    ...
    local data = {}
    for _, field in ipairs({'FontColor', 'MainColor', 'AccentColor', 'BackgroundColor', 'OutlineColor'}) do
        data[field] = Options[field].Value:ToHex()
    end
    writefile(self.Folder .. '/themes/' .. name .. '.json', HttpService:JSONEncode(data))
end
function ThemeManager:ReloadCustomThemes()
    local out = {}
    for _, path in ipairs(listfiles(self.Folder .. '/themes')) do
...
function ThemeManager:CreateThemeManager(groupbox)
    for _, data in ipairs({
        {'Background color', 'BackgroundColor'}, {'Main color', 'MainColor'},
        {'Accent color', 'AccentColor'}, {'Outline color', 'OutlineColor'}, {'Font color', 'FontColor'},
    }) do
        groupbox:AddLabel(data[1]):AddColorPicker(data[2], {Default = self.Library[data[2]]})
    end
    local values = {}
    for name, data in pairs(self.BuiltInThemes) do values[data[1]] = name end
    local themes = {}
    for i = 1, #values do themes[i] = values[i] end
    groupbox:AddDivider()
    ...
    end):AddButton('Load theme', function() self:ApplyTheme(Options.ThemeManager_CustomThemeList.Value) end)
    ...
    groupbox:AddButton('Set custom default', function()
        ...
        if name and name ~= '' then self:SaveDefault(name) end
    end)
    self:LoadDefault()
    for _, field in ipairs({'BackgroundColor', 'MainColor', 'AccentColor', 'OutlineColor', 'FontColor'}) do
        Options[field]:OnChanged(function() self:ThemeUpdate() end)
    end
end
...
SaveManager.Parser.Toggle = {Save=function(i,o) return {type='Toggle',idx=i,value=o.Value} end, Load=function(i,d) if Toggles[i] then Toggles[i]:SetValue(d.value) end end}
SaveManager.Parser.Slider = {Save=function(i,o) return {type='Slider',idx=i,value=tostring(o.Value)} end, Load=function(i,d) if Options[i] then Options[i]:SetValue(d.value) end end}
SaveManager.Parser.Dropdown = {Save=function(i,o) return {type='Dropdown',idx=i,value=o.Value,multi=o.Multi} end, Load=function(i,d) if Options[i] then Options[i]:SetValue(d.value) end end}
SaveManager.Parser.ColorPicker = {Save=function(i,o) return {type='ColorPicker',idx=i,value=o.Value:ToHex(),transparency=o.Transparency} end, Load=function(i,d) if Options[i] then Options[i]:SetValueRGB(Color3.fromHex(d.value),d.transparency) end end}
SaveManager.Parser.KeyPicker = {Save=function(i,o) return {type='KeyPicker',idx=i,mode=o.Mode,key=o.Value} end, Load=function(i,d) if Options[i] then Options[i]:SetValue({d.key,d.mode}) end end}
SaveManager.Parser.Input = {Save=function(i,o) return {type='Input',idx=i,text=o.Value} end, Load=function(i,d) if Options[i] then Options[i]:SetValue(d.text) end end}
...
function SaveManager:SetIgnoreIndexes(list) for _, key in ipairs(list) do self.Ignore[key] = true end end
function SaveManager:IgnoreThemeSettings() self:SetIgnoreIndexes({'BackgroundColor','MainColor','AccentColor','OutlineColor','FontColor','ThemeManager_ThemeList','ThemeManager_CustomThemeList','ThemeManager_CustomThemeName'}) end
function SaveManager:BuildFolderTree()
    for _, path in ipairs({self.Folder, self.Folder .. '/themes', self.Folder .. '/settings'}) do if not isfolder(path) then makefolder(path) end end
end
function SaveManager:Save(name)
    if type(name) ~= 'string' or name:gsub('%s','') == '' then return false, 'no config file is selected' end
    local data = {objects={}}
    for idx, obj in pairs(Toggles) do if not self.Ignore[idx] and self.Parser[obj.Type] then data.objects[#data.objects+1] = self.Parser[obj.Type].Save(idx,obj) end end
    for idx, obj in pairs(Options) do if not self.Ignore[idx] and self.Parser[obj.Type] then data.objects[#data.objects+1] = self.Parser[obj.Type].Save(idx,obj) end end
    local ok, encoded = pcall(HttpService.JSONEncode, HttpService, data)
    if not ok then return false, 'failed to encode data' end
    local okWrite = pcall(writefile, self.Folder .. '/settings/' .. name .. '.json', encoded)
    return okWrite, okWrite and nil or 'failed to write config'
end
function SaveManager:Load(name)
    ...
    local ok, data = pcall(HttpService.JSONDecode, HttpService, readfile(path))
    if not ok or type(data) ~= 'table' or type(data.objects) ~= 'table' then return false, 'decode error' end
    for _, item in ipairs(data.objects) do if self.Parser[item.type] then task.spawn(self.Parser[item.type].Load, item.idx, item) end end
    return true
end
function SaveManager:Delete(name)
    ...
    if not ok then return false, 'failed to delete config' end
    local autoload = self.Folder .. '/settings/autoload.txt'
    if isfile(autoload) and readfile(autoload) == name then pcall(delfile, autoload) end
    return true
end
function SaveManager:RefreshConfigList()
    local out = {}
    for _, path in ipairs(listfiles(self.Folder .. '/settings')) do
        if path:sub(-5) == '.json' then local name = path:match('([^/\\]+)%.json$') if name then out[#out+1] = name end end
    end
    return out
end
function SaveManager:LoadAutoloadConfig()
    ...
    if isfile(path) then
        local ok, err = self:Load(readfile(path))
        if not ok and self.Library then self.Library:Notify('Failed to load autoload config: ' .. err, 2) end
        return ok, err
    end
end
function SaveManager:BuildConfigSection(tab)
    ...
    section:AddButton('Create config', function()
        ...
        if not ok then return self.Library:Notify('Failed to save config: ' .. err, 2) end
        self.Library:Notify(string.format('Created config %q', name))
        Options.SaveManager_ConfigList:SetValues(self:RefreshConfigList())
        Options.SaveManager_ConfigList:SetValue(nil)
    end):AddButton('Load config', function()
        local ok, err = self:Load(Options.SaveManager_ConfigList.Value)
    ...
    section:AddButton('Overwrite config', function()
        ...
        self.Library:Notify('Overwrote config', 2)
    end)
    ...
    section:AddButton('Set as autoload', function()
        local name = Options.SaveManager_ConfigList.Value
        if type(name) ~= 'string' or name:gsub('%s', '') == '' then
            return self.Library:Notify('No config is selected', 2)
        end
