...
local ScriptVersion = "v1.5.9.7b"
local ScriptStatus = "Undetected"


local CONSTANTS = {
    walkSpeed = 16,
    repo = "https://raw.githubusercontent.com/no1setz2/obsidian-for-valenok/refs/heads/main/",
    skinPath = "Valenok_skin/Skin.json",
    maxHitChams = 25,
    espBoxTop = 2.45,
    espBoxBottom = -3.1,
    espBoxThickness = 1,
    ...
    hitboxPriority = { "Head", "Body", "Arms", "Legs" },
    rageHitboxes = { Head = true },
    maxWalls = 3,
    hitboxNames = {
...
for _, name in ipairs(CONSTANTS.hitboxNames) do
    CONSTANTS.hitboxLookup[name] = true
end

local Library, ThemeManager, SaveManager

local success, err = pcall(function()
    local repo = CONSTANTS.repo
...
local EspFrameCache = {
    ...
    colors = {},
    boxFillAlpha = 1,
    chamsVisibleAlpha = 0.35,
    chamsWallAlpha = 0.35,
}

local EspPlayerCache = {}

local function invalidateEspCache(player)
...
task.spawn(function()
local success, err = pcall(function()
    local gameVersionObj = ReplicatedStorage:WaitForChild("GameVersion")
    RuntimePack.CbVersion = gameVersionObj.Value
end)

if success then
else
    RuntimePack.GameVersion = ""
    RuntimePack.CbVersion = ""
end
end)
...
local function clientEnv()
    return Cache:getOrSet("Client", 1, function()
        local pg = getPlayerGui()
...
local rayIgnoreCache = { list = nil, t = 0 }
RuntimePack.ModeCache = { t = 0, value = false }
RuntimePack.TeamIgnoreCache = { t = 0, value = false }
RuntimePack.RayIgnoreMemo = {}
RuntimePack.IgnoreRootsCache = {}
RuntimePack.CharIgnorePartsCache = {}
local enemyIgnoreNames = {
...
for i = 1, 15 do
    enemyIgnoreNames["Hat" .. i] = true
end

RuntimePack.invalidateCharIgnoreParts = function(character)
    if character then RuntimePack.CharIgnorePartsCache[character] = nil end
end

local function isCompMode()
    local now = tick()
    if now - RuntimePack.ModeCache.t < 1 then return RuntimePack.ModeCache.value end
    RuntimePack.ModeCache.t = now
    ...
    if not status then
        RuntimePack.ModeCache.value = false
        return false
    end
    ...
    RuntimePack.ModeCache.value = matched and true or false
    return RuntimePack.ModeCache.value
end
...
local function isTriggerTeamCheck()
    local option = Options and Options.TriggerbotChecks
    local checks = option and option.Value
    if type(checks) ~= "table" then return false end
    if checks.Team == true then return true end
...
local function isAimTeamCheck()
    local option = Options and Options.AimbotChecks
    local checks = option and option.Value
    if type(checks) ~= "table" then return false end
    if checks.Team == true then return true end
    for _, v in pairs(checks) do
...
local function shouldRayIgnoreTeammates()
    local now = tick()
    if now - RuntimePack.TeamIgnoreCache.t < 0.25 then return RuntimePack.TeamIgnoreCache.value end
    RuntimePack.TeamIgnoreCache.t = now

    local anyCombatCheck = false
    local anyUnchecked = false

    local function consider(enabled, check)
    ...
    consider(Toggles.AimbotEnable and Toggles.AimbotEnable.Value, isAimTeamCheck())
    consider(Toggles.TriggerbotEnable and Toggles.TriggerbotEnable.Value, isTriggerTeamCheck())
    consider(Toggles.RagebotEnable and Toggles.RagebotEnable.Value, Toggles.RagebotTeamCheck and Toggles.RagebotTeamCheck.Value)
    consider(Toggles.SilentAimEnable and Toggles.SilentAimEnable.Value, Toggles.SilentAimTeamCheck and Toggles.SilentAimTeamCheck.Value)

    local value
    if anyCombatCheck then
        value = not anyUnchecked
    else
        value = not isCompMode()
    end

    RuntimePack.TeamIgnoreCache.value = value
    return value
end

local function appendEnemyIgnoreParts(list, character)
    if not character then return end
    local cached = RuntimePack.CharIgnorePartsCache[character]
    if cached then
        if not character.Parent then
            RuntimePack.CharIgnorePartsCache[character] = nil
            return
        end
        for i = 1, #cached do
            local part = cached[i]
    ...
    RuntimePack.CharIgnorePartsCache[character] = parts
    for i = 1, #parts do
        table.insert(list, parts[i])
    end
end

RuntimePack.rebuildIgnoreRoots = function()
    table.clear(RuntimePack.IgnoreRootsCache)
    ...
    if rayIgnore then RuntimePack.IgnoreRootsCache[#RuntimePack.IgnoreRootsCache + 1] = rayIgnore end
    if debris then RuntimePack.IgnoreRootsCache[#RuntimePack.IgnoreRootsCache + 1] = debris end
    if clips then RuntimePack.IgnoreRootsCache[#RuntimePack.IgnoreRootsCache + 1] = clips end
    if spawns then RuntimePack.IgnoreRootsCache[#RuntimePack.IgnoreRootsCache + 1] = spawns end
    if cam then RuntimePack.IgnoreRootsCache[#RuntimePack.IgnoreRootsCache + 1] = cam end
    if localChar then RuntimePack.IgnoreRootsCache[#RuntimePack.IgnoreRootsCache + 1] = localChar end
end
...
local function buildIgnoreList()
    ...
    local cached = rayIgnoreCache.list
    if cached
        and (now - rayIgnoreCache.t) < 0.015
        and rayIgnoreCache.ignoreTeammates == ignoreFullTeammates
    then
        return cached
    end

    table.clear(RuntimePack.RayIgnoreMemo)
    RuntimePack.rebuildIgnoreRoots()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        ...
        if ignoreFullTeammates and isSameTeamPlayer(player) then
            ...
            appendEnemyIgnoreParts(list, pChar)
        end
    end

    rayIgnoreCache.list = list
    rayIgnoreCache.t = now
    rayIgnoreCache.ignoreTeammates = ignoreFullTeammates
    return list
end

local function isUnderRayIgnore(inst)
    if not inst then return false end
    local memo = RuntimePack.RayIgnoreMemo[inst]
    if memo ~= nil then return memo end

    local roots = RuntimePack.IgnoreRootsCache
    for i = 1, #roots do
        local root = roots[i]
        if root and (inst == root or inst:IsDescendantOf(root)) then
            RuntimePack.RayIgnoreMemo[inst] = true
            return true
        end
    end

    if enemyIgnoreNames[inst.Name] then
    ...
    RuntimePack.RayIgnoreMemo[inst] = false
    return false
end
...
local function copyIgnoreList()
    local base = buildIgnoreList()
...
local function controlTurnRemote()
...
local function isEnemy(player)
    if player == LocalPlayer then return false end
    if isAimTeamCheck() and isSameTeamPlayer(player) then return false end
    return true
end

local function isTriggerEnemy(player)
    if player == LocalPlayer then return false end
    if isTriggerTeamCheck() and isSameTeamPlayer(player) then return false end
    return true
end
...
local function getChamsAlpha(optionName, fallback)
...
local function createText()
    ...
    t.Size, t.Font = CONSTANTS.espTextSize, CONSTANTS.espFont
    return t
end
...
local function screenBox(character, humanoid, rootPart)
    if not rootPart then return nil end
    local camera = getCamera()
    if not camera then return nil end
    local rootPos = rootPart.Position
    local top = Vector3.new(rootPos.X, rootPos.Y + CONSTANTS.espBoxTop, rootPos.Z)
    local bottom = Vector3.new(rootPos.X, rootPos.Y + CONSTANTS.espBoxBottom, rootPos.Z)
...
local function equippedToolName(player, character)
    if not character then return "" end
...
function CombatScan.maxWallsAllowed()
    ...
    return Options.SilentAimMaxWalls and Options.SilentAimMaxWalls.Value or CONSTANTS.maxWalls
end
...
function CombatScan.clear()
    ...
    CombatScan.silentPart = nil
    CombatScan.silentPoint = nil
    CombatScan.origin = nil
end

local multiPoints = table.create(7)
local singlePoint = table.create(1)
local magnetParts = { "Head", "HeadHB", "HumanoidRootPart", "UpperTorso", "Torso" }

function CombatScan.getPoints(part)
    if not part then return nil end
    local cf = part.CFrame
    local sx, sy, sz = part.Size.X * 0.45, part.Size.Y * 0.45, part.Size.Z * 0.45
    multiPoints[1] = cf.Position
    multiPoints[2] = (cf * CFrame.new(sx, 0, 0)).Position
    multiPoints[3] = (cf * CFrame.new(-sx, 0, 0)).Position
    multiPoints[4] = (cf * CFrame.new(0, sy, 0)).Position
    multiPoints[5] = (cf * CFrame.new(0, -sy, 0)).Position
    multiPoints[6] = (cf * CFrame.new(0, 0, sz)).Position
    multiPoints[7] = (cf * CFrame.new(0, 0, -sz)).Position
    return multiPoints
end
...
function CombatScan.computeForwardTrackPoint(head, rootPart, character)
    ...
    forwardParams.FilterDescendantsInstances = ignore

    getgenv().IgnoreRaycastHook = true
    local ok, result = pcall(function()
        return Workspace:Raycast(head.Position, delta, forwardParams)
    end)
    getgenv().IgnoreRaycastHook = false
...
function CombatScan.silentWanted()
    return Toggles.SilentAimEnable
        and Toggles.SilentAimEnable.Value
        and isKeybindActive(Options.SilentAimKeybind)
end

function CombatScan.silentHitboxes()
    local opt = Options.SilentAimHitbox
    local value = opt and opt.Value
    if type(value) == "table" then return value end
    return { Head = true }
end

function CombatScan.getSilentParts(character, head, rootPart)
    local parts, seen = {}, {}
    ...
    local selected = CombatScan.silentHitboxes()

    for _, group in ipairs(CONSTANTS.hitboxPriority) do
        if CombatScan.rageHitboxOn(group, selected) then
            local names = CONSTANTS.hitboxFallbacks[group]
            for i = 1, #names do
...
function CombatScan.triggerHitboxOn(partName)
    ...
    for group, names in pairs(CONSTANTS.hitboxFallbacks) do
        if CombatScan.rageHitboxOn(group, selected) then
            for i = 1, #names do
...
local partsBuffer = table.create(20)
local seenParts = {}
local playerSort = table.create(32)
local entryPool = table.create(32)
for i = 1, 32 do
    playerSort[i] = {}
    entryPool[i] = {}
end
...
function CombatScan.collectParts(character, head, rootPart, needRage, needAim, needTrigger)
    ...
    if needTrigger then
        for i = 1, #magnetParts do addPart(findCharacterPart(character, magnetParts[i])) end
    end
    if #partsBuffer == 0 then addPart(head or rootPart) end
    return partsBuffer
end
...
function CombatScan.refresh(stamp)
    ...
    local needSilent = CombatScan.silentWanted()
    if not needRage and not needAim and not needTrigger and not needSilent then return end
    ...
    local bestRageWeight = -math.huge
    local bestSilentAngle = math.huge
    local bestSilentPart, bestSilentPoint = nil, nil

    local pCount = 0
    local top1Idx, top1Dist = nil, math.huge
    local top2Idx, top2Dist = nil, math.huge
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        ...
        local enemyAim = needAim and isEnemy(player)
        local enemyTrig = needTrigger and isTriggerEnemy(player)
        local enemySilent = needSilent and isEnemyFor(player, Toggles.SilentAimTeamCheck)
        if not enemyRage and not enemyAim and not enemyTrig and not enemySilent then continue end
        ...
        pCount = pCount + 1
        if not playerSort[pCount] then playerSort[pCount] = {} end
        local d = playerSort[pCount]
        d.player = player
        d.character = character
        d.humanoid = humanoid
        d.rootPart = rootPart
        d.head = head
        d.enemyRage = enemyRage
        d.enemyAim = enemyAim
        d.enemyTrig = enemyTrig
        d.enemySilent = enemySilent
        d.primaryPart = primaryPart
        d.primaryDist = primaryDist

        if primaryDist < top1Dist then
            top2Idx, top2Dist = top1Idx, top1Dist
            top1Idx, top1Dist = pCount, primaryDist
        elseif primaryDist < top2Dist then
            top2Idx, top2Dist = pCount, primaryDist
        end
    end

    local entryCount = 0
    for i = 1, pCount do
        local d = playerSort[i]
        local isTop2 = (i == top1Idx or i == top2Idx)
        
        local useMulti = allowMultiRay and isTop2 and (
            (d.enemyRage and Toggles.RagebotMultiPoint and Toggles.RagebotMultiPoint.Value)
            or (d.enemyAim and Toggles.AimbotMultiPoint and Toggles.AimbotMultiPoint.Value)
        )

        local bestPoint, bestWalls = d.primaryPart.Position, math.huge
        if isTop2 and d.primaryDist < 1.57 then
            bestPoint, bestWalls = CombatScan.findPoint(d.primaryPart, origin, maxWalls, useMulti)
            if not bestPoint then
                bestPoint = d.primaryPart.Position
                bestWalls = math.huge
            end
        end

        local canHit = bestWalls <= maxWalls
        if not canHit and d.enemyRage and isTop2 then
            local ftPoint, ftWalls = CombatScan.tryForwardTrackHit(d.head, d.rootPart, origin, d.character, maxWalls)
            if ftPoint then
                bestPoint = ftPoint
                bestWalls = ftWalls
                canHit = true
        ...
        if not entryPool[entryCount] then entryPool[entryCount] = {} end
        local entry = entryPool[entryCount]
        entry.player = d.player
        entry.character = d.character
        entry.humanoid = d.humanoid
        entry.root = d.rootPart
        entry.part = d.primaryPart
        entry.point = bestPoint
        entry.walls = bestWalls
        entry.canHit = canHit
        entry.strictPart = bestWalls == 0 and d.primaryPart or nil
        entry.strictPoint = bestWalls == 0 and bestPoint or nil
        entry.mouseDist = angDist

        CombatScan.byPlayer[d.player] = entry
        CombatScan.list[#CombatScan.list + 1] = entry

        if d.enemyRage and entry.canHit then
            local weight = 0
            if entry.walls == 0 then weight = weight + 1000 end
            if entry.walls > 0 and entry.walls <= maxWalls then weight = weight + 500 end
            weight = weight + (1000 - math.clamp(mag, 0, 1000))
            weight = weight + (100 - d.humanoid.Health)
            
            if weight > bestRageWeight then
                bestRageWeight = weight
        ...
        if d.enemySilent and isTop2 then
            local silentVisible = Toggles.SilentAimVisibleCheck and Toggles.SilentAimVisibleCheck.Value
            local silentFov = Options.SilentAimFOV and Options.SilentAimFOV.Value or 45
            silentFov = type(silentFov) == "number" and math.clamp(silentFov, 1, 180) or 45
            local silentMaxAngle = silentFov >= 180 and math.huge or math.rad(silentFov * 0.5)
            local silentParts = CombatScan.getSilentParts(d.character, d.head, d.rootPart)

            for j = 1, #silentParts do
                local part = silentParts[j]
                local point = part.Position
                local deltaS = point - origin
                local magS = deltaS.Magnitude

                if magS > 1e-4 then
                    local angle = math.acos(math.clamp(camLook:Dot(deltaS / magS), -1, 1))

                    if angle <= silentMaxAngle and angle < bestSilentAngle then
                        local visible = true
                        if silentVisible then
                            visible = getWallCount(origin, point, 0, d.character) == 0
                        end
                        if visible then
                            bestSilentPart = part
                            bestSilentPoint = point
                            bestSilentAngle = angle
                        end
                    end
                end
            end
    ...
    if bestSilentPart then
        CombatScan.silentPart = bestSilentPart
        CombatScan.silentPoint = bestSilentPoint
    end
...
function RageHitLog.push(message, key, kind)
    ...
    if key and key == RageHitLog.LastKey and now - RageHitLog.LastTime < 0.08 then
        return
    end
...
function RageHitLog.extractReason(raw)
    ...
    local tagStart = lower:find("[hitreg feedback]", 1, true)
        or lower:find("[hitger feedback]", 1, true)
...
function RageHitLog.handleRemoteFeedback(msg)
    ...
    local reason = RageHitLog.extractReason(msg)
        or (msg ~= nil and tostring(msg):match("^%s*(.-)%s*$"))
...
function RageHitLog.installFeedbackHook()
    ...
    task.spawn(function()
        ...
        local remote = events:FindFirstChild("DebugFeedback")
            or events:WaitForChild("DebugFeedback", 10)
...
function RageHitLog.draw(center)
    RageHitLog.ensure()
    local now = tick()
    if not RageHitLog.enabled() or not center then
        for i = 1, RageHitLog.MaxEntries do
...
do
    ...
    local function refreshHitpart(now)
        ...
        if gun and eq then
            ...
            if gunName ~= HitpartSilent.gunName or HitpartSilent.charGun ~= gun then
                ...
                HitpartSilent.fireRateRefreshedAt = 0
            end
        else
    ...
    HitpartSilent.getFireRate = function()
        local now = tick()
        if now - HitpartSilent.fireRateRefreshedAt >= 0.1 then
            HitpartSilent.fireRateRefreshedAt = now
            local char = LocalPlayer.Character
            local gun = char and char:FindFirstChild("Gun")
            local eq = char and char:FindFirstChild("EquippedTool")
            if gun and eq then
    ...
    HitpartSilent.fire = function(target, aimPoint)
        ...
        if not RuntimePack.rageSilentActive and not RuntimePack.silentAimActive then return false end
        ...
        local walls, canHit
        if RuntimePack.rageSilentActive then
            ...
            walls = getWallCount(camPos, hitPos, 0, targetChar)
            canHit = not (Toggles.SilentAimVisibleCheck
                and Toggles.SilentAimVisibleCheck.Value
                and walls > 0)
        end
...
local function isLookingAtEnemy()
    ...
    local _, humanoid = characterParts(plr)
...
local function updateAutoScope()
    ...
    if not legitOn and not rageOn then
        if AutoScopeState.lastWant then
            setADS(clientEnv(), false)
            AutoScopeState.lastWant = false
        end
        return
    end

    local client = clientEnv()
    ...
    local want = false
    if legitOn and isLookingAtEnemy() then
        want = true
    elseif rageOn and RuntimePack.rageSilentActive then
...
local function ensureTracerFolder()
    ...
    tracerState.Folder = f
    return f
end

local function clearTracers()
    for _, slot in pairs(tracerState.Pool) do
        if slot.beam then slot.beam.Enabled = false end
    end
    if tracerState.Folder then pcall(function() tracerState.Folder:Destroy() end); tracerState.Folder = nil end
    local leftover = Workspace:FindFirstChild("ValenokBulletTracers")
    if leftover then pcall(function() leftover:Destroy() end) end
    table.clear(tracerState.Pool)
    tracerState.Index = 1
end

local function tracerMuzzle(fallback)
    local cam = getCamera()
...
local function tracerAnchor(name, parent)
...
RuntimePack.getTracerSlot = function()
    local slot = tracerState.Pool[tracerState.Index]
    tracerState.Index = (tracerState.Index % tracerState.Size) + 1
    if not slot then
        local folder = ensureTracerFolder()
        slot = {
            p0 = tracerAnchor("TracerStart", folder),
            p1 = tracerAnchor("TracerEnd", folder),
        ...
        tracerState.Pool[#tracerState.Pool + 1] = slot
    end
    return slot
end

RuntimePack.updateBulletTracers = function(now)
    for _, slot in ipairs(tracerState.Pool) do
...
drawBulletTracer = function(startPos, endPos)
    ...
    startPos = tracerMuzzle(startPos)
    ...
    local texId = CONSTANTS.tracerTextures[texName] or CONSTANTS.tracerTextures.Solid
    local texture = type(texId) == "number" and ("rbxassetid://" .. texId) or texId

    local slot = RuntimePack.getTracerSlot()
    ...
    beam.Enabled = true
    slot.expire = tick() + tracerState.Lifetime
end
...
local function ensureNadePreview()
    ...
    for i = 1, 150 do
        ...
        if i > 1 then
            ...
            beam.Width0, beam.Width1, beam.FaceCamera, beam.Segments = 0.06, 0.06, true, 1
...
local function hideNadePreview()
...
local function updateNadePreview(dt)
    ...
    local lp = LocalPlayer
    local _, _, hrp = characterParts(lp)
    ...
    local params = CONSTANTS.grenadeParams[nadeType] or CONSTANTS.grenadeParams.default
    local maxBounces = params.maxBounces
    local bounceDamping = params.bounceDamping
    local velocity = cam.CFrame.LookVector * CONSTANTS.grenadeParams.LOOK_SPEED + plrVel * CONSTANTS.grenadeParams.PLR_FACTOR + Vector3.new(0, CONSTANTS.grenadeParams.UP_BIAS, 0)
    ...
    for _ = 1, maxSteps do
        ...
        if ray then
            ...
            if (nadeType == "molotov" and isFloor) or bounces >= maxBounces or velocity.Magnitude < 5 then
                if pointCount < 150 then
                    ...
                    if beam then beam.Transparency = NumberSequence.new(0.15 + (pointCount / 150) * 0.85) end
                end
                currentPos = nextPos
                break
            end
        ...
        if stepIdx % samplePeriod == 0 or ray then
            if pointCount >= 150 then break end
            pointCount = pointCount + 1
            local att = GrenadeRuntime.Attachments[pointCount]
            local beam = GrenadeRuntime.Beams[pointCount - 1]
            if not att then
    ...
    for j = 1, pointCount do
        ...
        if att then
            cachedPoints[#cachedPoints + 1] = {
                ...
                transparency = 0.15 + (j / 150) * 0.85,
                sequence = NumberSequence.new(0.15 + (j / 150) * 0.85),
            }
        end
    end
...
RuntimePack.ShowHitMarker = function()
    ensureHitMarker()
    local cam = getCamera()
    ...
    for i, seg in ipairs(segs) do
        ...
        if ol then ol.From, ol.To, ol.Thickness, ol.Color, ol.Transparency, ol.Visible = from - unit, to + unit, th + 2, Color3.new(), 0, true end
        if fl then fl.From, fl.To, fl.Thickness, fl.Color, fl.Transparency, fl.Visible = from, to, th, color, 0, true end
    end
    ...
    if not HitMarkerState.HeartbeatConn then
        HitMarkerState.HeartbeatConn = RunService.Heartbeat:Connect(function()
            ...
            local progress = math.clamp((now - HitMarkerState.HoldUntil) / HitMarkerState.FadeDuration, 0, 1)
            for _, obj in ipairs(HitMarkerState.OutlineLines) do if obj then obj.Transparency = progress end end
            for _, obj in ipairs(HitMarkerState.FillLines) do if obj then obj.Transparency = progress end end
            if progress >= 1 then
...
RuntimePack.PlayHitSound = function()
...
local function updateEspFrameCache()
    ...
    EspFrameCache.boxFillAlpha = (fillOpt and fillOpt.Transparency) and math.clamp(1 - fillOpt.Transparency, 0, 1) or 1
    EspFrameCache.chamsVisibleAlpha = getChamsAlpha("ESPChamsVisibleColor", 0.35)
    EspFrameCache.chamsWallAlpha = getChamsAlpha("ESPChamsWallColor", 0.35)
end
...
local function findAmmoInClient()
    local client = clientEnv and clientEnv() or nil
...
local function requestAmmoScan(force)
...
if not InfAmmoState.charConn then
    InfAmmoState.charConn = LocalPlayer.CharacterAdded:Connect(function()
        ...
        if Toggles and Toggles.ExploitInfAmmo and Toggles.ExploitInfAmmo.Value then
            task.defer(function()
                requestAmmoScan(true)
            end)
        end
    end)
end

local updateBhop, updateLegitBhop, updateThirdPerson, updateTpNoClip, updateNoclip, updateFly, updateAutoJump, updateAutoCrouch, updateSpeedHack, updateFakeDuck
...
RuntimePack.ensurePeekDrawings = function()
...
local function drawPeekCircle(cam, worldPos)
    RuntimePack.ensurePeekDrawings()
    local RADIUS = 2.4
...
local function retreatToSaved(savedCF)
    ...
    task.spawn(function()
        local char = LocalPlayer.Character
        local _, hum, hrp = characterParts(LocalPlayer)
        if hrp then
...
local function updatePeekAssist()
    ...
    local char     = LocalPlayer.Character
    local _, humanoid, hrp = characterParts(LocalPlayer)
    ...
    if PeekAssist.SavedCFrame and cam then
        ...
        if hrp then
            ...
            PeekWallParams.FilterDescendantsInstances = buildIgnoreList()
...
RuntimePack.peekAssistOnShot = function()
...
local instaWeapon = { SavedEquipTimes = {}, SavedReloadTimes = {} }
local recoilBase, rcsBase = {}, {}
local baseAccuracySd
...
local function getSilentAimFov()
    local fovValue = Options.SilentAimFOV and Options.SilentAimFOV.Value
    if type(fovValue) ~= "number" then return 45 end
    return math.clamp(fovValue, 1, 180)
end

local function getSilentAimFovRadius()
    local silentFov = getSilentAimFov()
    if silentFov >= 180 then return 999999 end
    local cam = getCamera()
    if not cam then return 0 end
    local halfViewport = cam.ViewportSize.Y * 0.5
    local camFovHalfRad = math.rad(cam.FieldOfView * 0.5)
    local silentFovHalfRad = math.rad(silentFov * 0.5)
    return (math.tan(silentFovHalfRad) / math.tan(camFovHalfRad)) * halfViewport
end

local function hasAimCheck(name)
    local option = Options.AimbotChecks
    local checks = option and option.Value
    if type(checks) ~= "table" then return false end
    if checks[name] == true then return true end
...
local AimSmokeParams = RaycastParams.new()
AimSmokeParams.FilterType = Enum.RaycastFilterType.Include
local AimSmokeFilter = table.create(1)

local function aimSmokeBlocks(origin, targetPos)
    local rayIgnore = Workspace:FindFirstChild("Ray_Ignore")
    local smokes = rayIgnore and rayIgnore:FindFirstChild("Smokes")
    if not smokes then return false end
    AimSmokeFilter[1] = smokes
    AimSmokeParams.FilterDescendantsInstances = AimSmokeFilter
    getgenv().IgnoreRaycastHook = true
    local result = Workspace:Raycast(origin, targetPos - origin, AimSmokeParams)
    getgenv().IgnoreRaycastHook = false
    return result ~= nil
end

local function isAimFlashed()
    local pg = getPlayerGui()
    local blnd = pg and pg:FindFirstChild("Blnd")
    local blind = blnd and blnd:FindFirstChild("Blind")
    return blind ~= nil and blind.BackgroundTransparency < 0.4
end

local function isAimInAir()
    local _, humanoid = characterParts(LocalPlayer)
    if not humanoid then return false end
    local ok, state = pcall(function() return humanoid:GetState() end)
    if ok and state == Enum.HumanoidStateType.Freefall then return true end
    return humanoid.FloorMaterial == Enum.Material.Air
end

local function isAimScopedOut()
    local client = clientEnv()
    local gun = client and rawget(client, "gun")
    local isScopedWeapon = false
    if typeof(gun) == "Instance" then
        isScopedWeapon = gun:FindFirstChild("Scoped") ~= nil or gun:FindFirstChild("snipo") ~= nil
    end
    if not isScopedWeapon then return false end
    local pg = getPlayerGui()
    local gui = pg and pg:FindFirstChild("GUI")
    local cross = gui and gui:FindFirstChild("Crosshairs")
    local scope = cross and cross:FindFirstChild("Scope")
    return scope ~= nil and not scope.Visible
end
...
local function getClosestAimTarget(screenCenter, fovRadius)
    ...
    local useVisible = hasAimCheck("Visible")
    local useSmoke = hasAimCheck("Smoke")
    local useTeam = hasAimCheck("Team")
    local aimFov = getAimFov()
    ...
    for i = 1, #CombatScan.list do
        ...
        if useTeam and isSameTeamPlayer(entry.player) then continue end
        ...
        if useSmoke and aimSmokeBlocks(camPos, aimPoint) then continue end
...
local function updateAimBot(dt)
    ...
    if hasAimCheck("Flash") and isAimFlashed() then return end
    if hasAimCheck("Air") and isAimInAir() then return end
    if hasAimCheck("Scope") and isAimScopedOut() then return end
...
;(function()
local HasOption = function(value, name)
...
local function GetTriggerChecks()
    local option = Options.TriggerbotChecks
...
local function GetTriggerHitbox()
    local option = Options.TriggerbotHitbox
    local value = option and option.Value
    if type(value) == "table" then
        return value
    end
...
local function GetWeapRemote()
    local events = ReplicatedStorage:FindFirstChild("Events")
    return events and events:FindFirstChild("weap")
end

local WeaponStatsCache = {}
local function GetWeaponStats()
    local char = LocalPlayer.Character
    local name = char and equippedToolName(LocalPlayer, char) or ""
    local cached = WeaponStatsCache[name]
    if cached then
        return cached.fireRate, cached.range, cached.penetration
    end
    local weapons = getWeaponsFolder()
    local stats = (name ~= "" and weapons and weapons:FindFirstChild(name)) or nil
    if not stats and char and weapons then
        local gun = char:FindFirstChild("Gun") or char:FindFirstChild("Gun2")
        if gun then
            stats = weapons:FindFirstChild(gun.Name)
        end
    end
    local fireRate, range = 0.1, 64
    if stats then
        local fr = stats:FindFirstChild("FireRate")
        if fr and type(fr.Value) == "number" and fr.Value > 0 then
            fireRate = fr.Value
        end
        local rg = stats:FindFirstChild("Range")
        if rg and type(rg.Value) == "number" and rg.Value > 0 then
            range = math.clamp(rg.Value, 1, 100)
        end
    end
    local penSrc = stats
    if penSrc then
        local copyFrom = penSrc:FindFirstChild("CopyFrom")
        if copyFrom then
            local ref = copyFrom.Value
            if typeof(ref) == "Instance" then
                penSrc = ref
            elseif type(ref) == "string" and ref ~= "" and weapons then
                penSrc = weapons:FindFirstChild(ref) or penSrc
            end
        end
    end
    local gun = char and (char:FindFirstChild("Gun") or char:FindFirstChild("Gun2"))
    local pen = (penSrc and penSrc:FindFirstChild("Penetration")) or (gun and gun:FindFirstChild("Penetration"))
    local penetration = 0
    if pen and type(pen.Value) == "number" then
        penetration = math.max(pen.Value, 0) * 0.01
    end
    WeaponStatsCache[name] = {fireRate = fireRate, range = range, penetration = penetration}
    return fireRate, range, penetration
end

local function GetTriggerWeaponInfo()
    local fireRate, range = GetWeaponStats()
...
local function GetSpreadAngle()
    ...
    local equipped = char:FindFirstChild("EquippedTool")
    ...
    if not spread or weapon:FindFirstChild("Melee") then
        return 0, nil
    end
    local client = clientEnv()
    ...
    if base <= 20 and not weapon:FindFirstChild("SMGThing") then
        base = base / 10
    end
...
local function GetSpreadPixels()
...
local function ComputeHitchance(part)
    if not part or not part.Parent or not part:IsA("BasePart") then
        return 0
    end
    local spreadAngle = GetSpreadAngle()
    if not spreadAngle or spreadAngle <= 1e-8 then
        return 100
    end
    local cam = getCamera()
    if not cam then
    ...
    local radius = 0.5 * math.max(part.Size.X, part.Size.Y, part.Size.Z)
    ...
    return math.clamp(targetAngle / spreadAngle, 0, 1) * 100
end
...
local function IsCombatRoundActive()
    local status = Workspace:FindFirstChild("Status")
    local prep = status and status:FindFirstChild("Preparation")
    local over = status and status:FindFirstChild("GameOver")
    return not (prep and prep.Value == true) and not (over and over.Value == true)
end

local function IsLocalFlashed()
    local ok, res = pcall(function()
        local gui = getPlayerGui()
        local blnd = gui and gui:FindFirstChild("Blnd")
        local blind = blnd and blnd:FindFirstChild("Blind")
        return blind ~= nil and blind.BackgroundTransparency < 0.4
    end)
...
local function IsLocalInAir()
    ...
    local ok2, mat = pcall(function()
        return hum.FloorMaterial
    end)
    return ok2 and mat == Enum.Material.Air
end

local function IsScopedOut()
    local ok, res = pcall(function()
        local char = LocalPlayer.Character
        if not char then
            return false
        ...
        if not (gun:FindFirstChild("snipo") or gun:FindFirstChild("Scoped")) then
            return false
        end
        local gui = getPlayerGui()
        local guiFrame = gui and gui:FindFirstChild("GUI")
        local cross = guiFrame and guiFrame:FindFirstChild("Crosshairs")
        local scope = cross and cross:FindFirstChild("Scope")
        return scope ~= nil and not scope.Visible
    end)
    return ok and res == true
...
local function IsSmokeBlocking(origin, targetPos)
    local ok, res = pcall(function()
        local ignore = Workspace:FindFirstChild("Ray_Ignore")
        local smokes = ignore and ignore:FindFirstChild("Smokes")
        if not smokes then
            return false
        end
        local params = RaycastParams.new()
        params.FilterType = Enum.RaycastFilterType.Include
        params.FilterDescendantsInstances = {smokes}
...
local function IsEffectPart(part)
    ...
    if string.find(n, "fire", 1, true) or string.find(n, "flame", 1, true)
        or string.find(n, "molotov", 1, true) or string.find(n, "incendiary", 1, true)
        or string.find(n, "burn", 1, true) or string.find(n, "smoke", 1, true) then
        return true
    end
    local rayIgnore = Workspace:FindFirstChild("Ray_Ignore")
    if rayIgnore and part:IsDescendantOf(rayIgnore) then
        return true
    end
    local debris = Workspace:FindFirstChild("Debris")
    if debris and part:IsDescendantOf(debris) then
        return true
    end
    return false
end

local function AddRaycastIgnores(filter)
    local rayIgnore = Workspace:FindFirstChild("Ray_Ignore")
    if rayIgnore then
        table.insert(filter, rayIgnore)
    end
    local debris = Workspace:FindFirstChild("Debris")
    if debris then
...
local function RaycastSkipEffects(origin, dir, excludeList, maxIter)
    local params = RaycastParams.new()
    ...
    local ok, hit = pcall(function()
        for _ = 1, (maxIter or 4) do
            local remaining = total - traveled
            if remaining <= 0 then
                return nil
            end
            local from = origin + dirUnit * traveled
            local h = Workspace:Raycast(from, dirUnit * remaining, params)
            if not h then
                return nil
            end
            if not IsEffectPart(h.Instance) then
                return h
            end
            traveled = traveled + (h.Position - from).Magnitude + 0.05
        end
        return nil
    end)
    if not ok then
...
local function IsPointVisible(origin, targetPos, targetChar)
    ...
    AddRaycastIgnores(filter)
    return RaycastSkipEffects(origin, targetPos - origin, filter) == nil
end
...
local function BuildTriggerIgnore(camera)
    table.clear(triggerIgnore)
    local function add(inst)
        if inst then
            triggerIgnore[#triggerIgnore + 1] = inst
        end
    end
    ...
    local char = LocalPlayer.Character
    if char then
        add(char:FindFirstChild("Gun"))
        add(char:FindFirstChild("Gun2"))
        add(char:FindFirstChild("BackC4"))
        add(char:FindFirstChild("Hitboxes"))
        add(char:FindFirstChild("FakeHead"))
        add(char:FindFirstChild("HeadHB"))
    end
    add(Workspace:FindFirstChild("Ray_Ignore"))
    add(Workspace:FindFirstChild("Debris"))
    local map = Workspace:FindFirstChild("Map")
    if map then
        add(map:FindFirstChild("Clips"))
        add(map:FindFirstChild("SpawnPoints"))
    end
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer then
            local c = p.Character
            local h = c and c:FindFirstChildOfClass("Humanoid")
            if c and (isSameTeamPlayer(p) or (h and h.Health <= 0)) then
                add(c)
            end
        end
    end
    triggerParams.FilterDescendantsInstances = triggerIgnore
...
local function GetTriggerWallFactor(part)
    ...
    if material == Enum.Material.DiamondPlate then
        factor = 3
    elseif material == Enum.Material.CorrodedMetal or material == Enum.Material.Metal
        or material == Enum.Material.Concrete or material == Enum.Material.Brick then
        factor = 2
    end
    if part.Name == "Grate" or material == Enum.Material.Wood or material == Enum.Material.WoodPlanks
        or (part.Parent and part.Parent:FindFirstChildOfClass("Humanoid")) then
        factor = 0.1
    end
    local rayIgnore = Workspace:FindFirstChild("Ray_Ignore")
    local debris = Workspace:FindFirstChild("Debris")
    if part.Transparency == 1 or part.CanCollide == false or part.Name == "Glass" or part.Name == "Cardboard"
        ...
        or (part.Parent and part.Parent.Name == "Hitboxes") then
        factor = 0
    end
...
local function getPenBudget()
    local _, _, penetration = GetWeaponStats()
    return penetration
end

local function GetTriggerWallThickness(part, hitPosition, direction, range)
    penInclude[1] = part
    penParams.FilterDescendantsInstances = penInclude
    local probeDirection = direction.Unit * range * 0.0625
    local result = Workspace:Raycast(hitPosition + probeDirection, probeDirection * -2, penParams)
    penInclude[1] = nil
    if result then
...
local function CanTriggerPenetrate(part, hitPosition, direction, used, wallCount, budget, range, autoWall)
    ...
    if not autoWall or wallCount >= 4 or budget <= 0 then
        return false, used, wallCount
    end
    used = math.min(budget, used + GetTriggerWallThickness(part, hitPosition, direction, range) * factor)
    if used >= budget then
        return false, used, wallCount
    end
...
local function IsTriggerHitboxPart(partName, hitSel)
    for group, names in pairs(CONSTANTS.hitboxFallbacks) do
        if HasOption(hitSel, group) then
...
local function GetTriggerTargetRay(camera, useTeam, hitSel, autoWall, range)
    BuildTriggerIgnore(camera)
    local origin = camera.CFrame.Position
    local direction = camera.CFrame.LookVector
    local rayEnd = origin + direction * 5000
    local penetrationUsed = 0
    local wallCount = 0
    local penBudget = getPenBudget()
    local part
    for _ = 1, 12 do
        local result = Workspace:Raycast(origin, rayEnd - origin, triggerParams)
        ...
        canPass, penetrationUsed, wallCount = CanTriggerPenetrate(
            part, result.Position, direction, penetrationUsed, wallCount, penBudget, range, autoWall)
        if not canPass then
            return nil
        end
        triggerIgnore[#triggerIgnore + 1] = part
        triggerParams.FilterDescendantsInstances = triggerIgnore
        origin = result.Position + direction * 0.05
        part = nil
    end
    ...
    local hum = char and char:FindFirstChildOfClass("Humanoid")
    if not player or player == LocalPlayer or not hum or hum.Health <= 0 then
        return nil
    end
    if useTeam and isSameTeamPlayer(player) then
        return nil
    end
    if HasCombatProtection(char) then
        return nil
    end
    if not IsTriggerHitboxPart(part.Name, hitSel) then
        return nil
    end
    return player, part
end

local function TriggerResetDelay()
...
fireWeapShot = FireWeapon
fireSingleShot = FireWeapon

TriggerbotState.getSpreadPixels = function()
    return GetSpreadPixels()
end

TriggerbotState.getCoveragePercent = function(targetPart)
    return ComputeHitchance(targetPart)
end
...
updateTriggerbot = function()
    ...
    local wanted = (Toggles.TriggerbotEnable and Toggles.TriggerbotEnable.Value)
        or isKeybindActive(Options.TriggerbotKeybind)
    if not wanted then
        TriggerResetDelay()
        return
    end
    ...
    local checks = GetTriggerChecks()
    if HasOption(checks, "Flash") and IsLocalFlashed() then
        TriggerResetDelay()
        return
    end
    if HasOption(checks, "Air") and IsLocalInAir() then
        TriggerResetDelay()
        return
    end
    if HasOption(checks, "Scope") and IsScopedOut() then
        TriggerResetDelay()
        return
    end
    local useSmoke = HasOption(checks, "Smoke")
    local useTeam = HasOption(checks, "Team")
    local useVisible = HasOption(checks, "Visible")

    if not IsCombatRoundActive() or HasCombatProtection(localChar) then
        TriggerResetDelay()
        return
    end

    local fireRate, range = GetTriggerWeaponInfo()
    local hitSel = GetTriggerHitbox()
    local autoWall = Toggles.TriggerbotAutoWall and Toggles.TriggerbotAutoWall.Value == true
    local threshold = tonumber(Options.TriggerbotHitChance and Options.TriggerbotHitChance.Value) or 50
    local delayMs = tonumber(Options.TriggerbotDelay and Options.TriggerbotDelay.Value) or 0

    local target, targetPart = GetTriggerTargetRay(camera, useTeam, hitSel, autoWall, range)
    if not target then
        TriggerResetDelay()
        return
    end
    if useVisible and targetPart then
        local targetChar = targetPart:FindFirstAncestorOfClass("Model")
        if not IsPointVisible(camera.CFrame.Position, targetPart.Position, targetChar) then
            TriggerResetDelay()
            return
        end
    end
    if useSmoke and targetPart and IsSmokeBlocking(camera.CFrame.Position, targetPart.Position) then
        TriggerResetDelay()
        return
    end
    if targetPart and ComputeHitchance(targetPart) + 1e-3 < threshold then
        TriggerResetDelay()
        return
    end
    ...
    local now = tick()
    if TriggerbotState.DelayTarget ~= target then
        TriggerbotState.DelayTarget = target
        TriggerbotState.DelayUntil = now + delayMs / 1000
    end
    if now < TriggerbotState.DelayUntil then
        return
    end
    if now - TriggerbotState.LastFire < fireRate + 0.01 then
        return
    end
    if FireWeapon() then
...
local function updateAntiAim()
    ...
    local _, humanoid, rootPart = characterParts(LocalPlayer)
    ...
    local waistPos = waist and waist.C0.Position or Vector3.new(0, 0.2, 0)
    local neckPos = neck and neck.C0.Position or Vector3.new(0, 0.8, 0)

    if not pitchEnabled and not yawEnabled then
        humanoid.AutoRotate = true
        if AntiAimState.HipHeight ~= nil then
            humanoid.HipHeight = AntiAimState.HipHeight
            AntiAimState.HipHeight = nil
        end
        if waist then waist.C0 = CFrame.new(waistPos) end
        if neck then neck.C0 = CFrame.new(neckPos) end
        return
    end

    if AntiAimState.HipHeight == nil then AntiAimState.HipHeight = humanoid.HipHeight end
    ...
    if pitchEnabled then
        ...
        if pitchMode ~= "None" then
            ...
            elseif pitchMode == "Custom" then
                pitchVal = Options.AntiAimPitchCustom and Options.AntiAimPitchCustom.Value or 0
            elseif pitchMode == "Random" then
                ...
                pitchVal = AntiAimState.PitchRandomAngle
            end

            if math.abs(pitchVal) <= 1 and pitchVal ~= 0 then
                pitchTarget = pitchVal * 50
            else
                pitchTarget = math.clamp(pitchVal, -50, 50)
            end

            local remote = controlTurnRemote()
            if remote then
                pcall(function() remote:FireServer(pitchVal) end)
            end
        end
    end

    if pitchEnabled and pitchTarget ~= 0 then
        if waist then
            waist.C0 = CFrame.new(waistPos) * CFrame.Angles(math.rad(pitchTarget), 0, 0)
        end
        if neck then
            neck.C0 = CFrame.new(neckPos)
        end
    else
        if waist then waist.C0 = CFrame.new(waistPos) end
        if neck then neck.C0 = CFrame.new(neckPos) end
    ...
    if yawEnabled then
        local yawMode = Options.AntiAimYawMode and Options.AntiAimYawMode.Value or "Local View"
        local baseYaw = 0

        if yawMode == "Local View" then
        ...
        local yawOffset = tonumber(Options.AntiAimYawOffset and Options.AntiAimYawOffset.Value) or 0

        local jitter = 0
        if Toggles.AntiAimYawJitterEnable and Toggles.AntiAimYawJitterEnable.Value then
            local jitterBase = Options.AntiAimYawJitterBase and Options.AntiAimYawJitterBase.Value or "offset"
            local jitterOffset = tonumber(Options.AntiAimYawJitterOffset and Options.AntiAimYawJitterOffset.Value) or 30

            if jitterBase == "offset" then
                if (tick() - AntiAimState.YawJitterLastSwitch) * 1000 >= 100 then
                    AntiAimState.YawJitterFlip = not AntiAimState.YawJitterFlip
                    AntiAimState.YawJitterLastSwitch = tick()
                end
                jitter = AntiAimState.YawJitterFlip and jitterOffset or -jitterOffset
            else
                local ways = (jitterBase == "5 way") and 5 or 3
                if (tick() - AntiAimState.YawJitterLastSwitch) * 1000 >= 100 then
                    AntiAimState.YawJitterIndex = (AntiAimState.YawJitterIndex + 1) % ways
                    AntiAimState.YawJitterLastSwitch = tick()
                end
                jitter = (AntiAimState.YawJitterIndex - (ways - 1) / 2) * jitterOffset
            end
        end

        local yawAngle = baseYaw + yawOffset + jitter
...
applyNoRecoil = function(enabled)
    local weapons = getWeaponsFolder()
    if not weapons then
        if not enabled then table.clear(recoilBase) end
        return
    end
    for _, weaponFolder in ipairs(weapons:GetChildren()) do
        ...
        if enabled then
            if recoilBase[weaponFolder.Name] == nil then
                recoilBase[weaponFolder.Name] = recoil.Value
            end
            recoil.Value = 1
        else
            local original = rcsBase[weaponFolder.Name] or recoilBase[weaponFolder.Name]
            if original ~= nil then
                recoil.Value = original
            end
            recoilBase[weaponFolder.Name] = nil
        end
    end
    if not enabled then
        table.clear(recoilBase)
        updateRCS()
    end
end
...
applyNoSpread = function(enabled)
    if enabled then
        local client = clientEnv()
        if not client then return end
        if baseAccuracySd == nil then
            baseAccuracySd = client.accuracy_sd
        end
        RuntimePack.NoSpreadClient = client
        client.accuracy_sd = 0
    else
        local client = RuntimePack.NoSpreadClient or clientEnv()
        if client then
            if baseAccuracySd ~= nil then
                client.accuracy_sd = baseAccuracySd
            elseif client.accuracy_sd == 0 then
                client.accuracy_sd = 0.001
            end
        end
        baseAccuracySd = nil
        RuntimePack.NoSpreadClient = nil
    end
end
...
applyInstaEquip = function(enabled)
    ...
    if enabled then
        for _, weaponFolder in ipairs(weapons:GetChildren()) do
            if weaponFolder:IsA("Folder") then
                ...
                if equipTime and equipTime:IsA("NumberValue") then
                    if instaWeapon.SavedEquipTimes[weaponFolder.Name] == nil then
                        instaWeapon.SavedEquipTimes[weaponFolder.Name] = equipTime.Value
                    end
                    equipTime.Value = 0
                end
            end
        ...
        for weaponName, original in pairs(instaWeapon.SavedEquipTimes) do
        ...
        table.clear(instaWeapon.SavedEquipTimes)
    end
end
...
applyInstaReload = function(enabled)
    ...
    if enabled then
        for _, weaponFolder in ipairs(weapons:GetChildren()) do
            if weaponFolder:IsA("Folder") then
                ...
                if reloadTime and reloadTime:IsA("NumberValue") then
                    if instaWeapon.SavedReloadTimes[weaponFolder.Name] == nil then
                        instaWeapon.SavedReloadTimes[weaponFolder.Name] = reloadTime.Value
                    end
        ...
        for weaponName, original in pairs(instaWeapon.SavedReloadTimes) do
            local weaponFolder = weapons:FindFirstChild(weaponName)
        ...
        table.clear(instaWeapon.SavedReloadTimes)
    end
end
...
local function getCurrentWeaponFireRateObject()
    ...
    local equippedName = getLocalEquippedTool()
    if equippedName then
        weaponName = tostring(equippedName.Value)
    end
...
updateRCS = function()
    ...
    for _, weaponFolder in ipairs(weapons:GetChildren()) do
        ...
        if rcsEnabled and rcsValue > 0 then
            if rcsBase[weaponFolder.Name] == nil then
                rcsBase[weaponFolder.Name] = recoil.Value
            end
            local original = rcsBase[weaponFolder.Name]
            local reduction = rcsValue / 100
            local newValue = original * (1 - reduction)
            recoil.Value = math.max(newValue, 1)
        else
            local original = rcsBase[weaponFolder.Name]
            if original ~= nil then
                recoil.Value = original
                rcsBase[weaponFolder.Name] = nil
            end
        end
    end
end
...
local killAllRemote
RuntimePack.killAllRemoteScanAt = 0
RuntimePack.resolveKillAllHitRemote = function()
    if killAllRemote and killAllRemote.Parent then return killAllRemote end
    killAllRemote = nil
    local now = tick()
    if now - RuntimePack.killAllRemoteScanAt < 1 then return nil end
    RuntimePack.killAllRemoteScanAt = now
    ...
    return killAllRemote
end

local function updateKillAll()
    local killAllEnabled = Toggles.ExploitKillAll and Toggles.ExploitKillAll.Value
    local killAllKey = isKeybindActive(Options.ExploitKillAllKeybind)

    local killPlayerEnabled = Toggles.ExploitKillPlayer and Toggles.ExploitKillPlayer.Value
    local killPlayerKey = isKeybindActive(Options.ExploitKillPlayerKeybind)

    local shouldKillAll = killAllEnabled and killAllKey
    local shouldKillPlayer = killPlayerEnabled and killPlayerKey

    if not shouldKillAll and not shouldKillPlayer then return end
    ...
    local gun = character:FindFirstChild("Gun")
    local equippedName = getLocalEquippedTool()
    if not gun or not equippedName then return end

    RuntimePack.resolveKillAllHitRemote()

    local gunName = "Scout"
    local gunRef = gun
    local replicatedStorageWeapons = getWeaponsFolder()
    local awpFolder = replicatedStorageWeapons and replicatedStorageWeapons:FindFirstChild("Scout")
    ...
    local targetPlayer = Options.ExploitKillTarget and Options.ExploitKillTarget.Value

    for _, plr in pairs(Players:GetPlayers()) do
        if plr == LocalPlayer then continue end

        if shouldKillPlayer then
            if not targetPlayer or (targetPlayer ~= plr and targetPlayer ~= plr.Name) then
                continue
            end
        end

        if shouldKillAll and not shouldKillPlayer then
        ...
        local _, playerHumanoid = characterParts(plr)
        ...
        if not killAllRemote then continue end
...
local nanGuard = { clientRef = nil, lastTry = 0, saved = nil }
...
local function patchNanGuard()
    local now = tick()
    if now - nanGuard.lastTry < 1.5 then return end
    nanGuard.lastTry = now

    local client = clientEnv()
    if not client then
        nanGuard.clientRef = nil
        return
    end
    if nanGuard.clientRef == client and client.__valenokNanGuard then
        return
    end
    ...
    pcall(function()
        ...
        nanGuard.saved = saved
        nanGuard.clientRef = client
    end)
end

nanGuard.restore = function()
    local saved = nanGuard.saved
...
task.spawn(function()
    for _ = 1, 20 do
        patchNanGuard()
        if nanGuard.clientRef and nanGuard.clientRef.__valenokNanGuard then
            break
        end
        task.wait(0.5)
    end
...
local function horizontalCameraDir(camCFrame, fwd, strafe)
...
RuntimePack.MoveUtil.applyCameraCFrameMove = function(rootPart, cam, speed, dt)
    local fwd, strafe = getMoveAxes()
    local direction = horizontalCameraDir(cam.CFrame, fwd, strafe)
    if not direction then return end

    rootPart.CFrame = rootPart.CFrame + direction * (speed * dt)
end

RuntimePack.MoveUtil.applyFlyMove = function(rootPart, camCFrame, speed, dt)
    local fwd, strafe = getMoveAxes()
...
RuntimePack.MoveUtil.localHumanoid = localHumanoid
RuntimePack.MoveUtil.aliveRig = aliveRig
RuntimePack.MoveUtil.MOVE_KEY_SPACE = MOVE_KEY_SPACE
RuntimePack.MoveUtil.MOVE_KEY_CTRL = MOVE_KEY_CTRL
RuntimePack.MoveUtil.AIR_MATERIAL = AIR_MATERIAL
RuntimePack.MoveUtil.ZERO_VECTOR = Vector3.new(0, 0, 0)
end)()

RuntimePack.MoveLoop = { rs = {}, hb = {}, st = {}, rsConn = nil, hbConn = nil, stConn = nil, rsN = 0, hbN = 0, stN = 0 }

RuntimePack.moveLoopEnsure = function(bucket)
    if bucket == "rs" then
        if RuntimePack.MoveLoop.rsConn then return end
        RuntimePack.MoveLoop.rsConn = RunService.RenderStepped:Connect(function(dt)
            local char, hum, hrp = RuntimePack.MoveUtil.aliveRig()
            for _, fn in pairs(RuntimePack.MoveLoop.rs) do pcall(fn, dt, char, hum, hrp) end
        end)
    elseif bucket == "hb" then
        if RuntimePack.MoveLoop.hbConn then return end
        RuntimePack.MoveLoop.hbConn = RunService.Heartbeat:Connect(function(dt)
            local char, hum, hrp = RuntimePack.MoveUtil.aliveRig()
            for _, fn in pairs(RuntimePack.MoveLoop.hb) do pcall(fn, dt, char, hum, hrp) end
        end)
    elseif bucket == "st" then
        if RuntimePack.MoveLoop.stConn then return end
        RuntimePack.MoveLoop.stConn = RunService.Stepped:Connect(function()
            local char, hum, hrp = RuntimePack.MoveUtil.aliveRig()
            for _, fn in pairs(RuntimePack.MoveLoop.st) do pcall(fn, 0, char, hum, hrp) end
        end)
    end
end

function RuntimePack.MoveLoop.set(bucket, key, fn)
    local map = RuntimePack.MoveLoop[bucket]
    if not map then return end
    local had = map[key] ~= nil
    if fn then
        map[key] = fn
        if not had then
            if bucket == "rs" then RuntimePack.MoveLoop.rsN = RuntimePack.MoveLoop.rsN + 1
            elseif bucket == "hb" then RuntimePack.MoveLoop.hbN = RuntimePack.MoveLoop.hbN + 1
            else RuntimePack.MoveLoop.stN = RuntimePack.MoveLoop.stN + 1 end
        end
        RuntimePack.moveLoopEnsure(bucket)
    elseif had then
        map[key] = nil
        if bucket == "rs" then
            RuntimePack.MoveLoop.rsN = math.max(0, RuntimePack.MoveLoop.rsN - 1)
            if RuntimePack.MoveLoop.rsN == 0 and RuntimePack.MoveLoop.rsConn then RuntimePack.MoveLoop.rsConn:Disconnect(); RuntimePack.MoveLoop.rsConn = nil end
        elseif bucket == "hb" then
            RuntimePack.MoveLoop.hbN = math.max(0, RuntimePack.MoveLoop.hbN - 1)
            if RuntimePack.MoveLoop.hbN == 0 and RuntimePack.MoveLoop.hbConn then RuntimePack.MoveLoop.hbConn:Disconnect(); RuntimePack.MoveLoop.hbConn = nil end
        else
            RuntimePack.MoveLoop.stN = math.max(0, RuntimePack.MoveLoop.stN - 1)
            if RuntimePack.MoveLoop.stN == 0 and RuntimePack.MoveLoop.stConn then RuntimePack.MoveLoop.stConn:Disconnect(); RuntimePack.MoveLoop.stConn = nil end
        end
    end
end

function RuntimePack.MoveLoop.clear()
    if RuntimePack.MoveLoop.rsConn then pcall(function() RuntimePack.MoveLoop.rsConn:Disconnect() end); RuntimePack.MoveLoop.rsConn = nil end
    if RuntimePack.MoveLoop.hbConn then pcall(function() RuntimePack.MoveLoop.hbConn:Disconnect() end); RuntimePack.MoveLoop.hbConn = nil end
    if RuntimePack.MoveLoop.stConn then pcall(function() RuntimePack.MoveLoop.stConn:Disconnect() end); RuntimePack.MoveLoop.stConn = nil end
    table.clear(RuntimePack.MoveLoop.rs); table.clear(RuntimePack.MoveLoop.hb); table.clear(RuntimePack.MoveLoop.st)
    RuntimePack.MoveLoop.rsN, RuntimePack.MoveLoop.hbN, RuntimePack.MoveLoop.stN = 0, 0, 0
end
...
Shared.restoreSpeedHackOriginal = function()
    ...
    if (not humanoid or not humanoid.Parent) then
        humanoid = RuntimePack.MoveUtil.localHumanoid()
    end
...
updateSpeedHack = function()
    RuntimePack.MoveLoop.set("rs", "SpeedHack", nil)
    ...
    RuntimePack.MoveLoop.set("rs", "SpeedHack", function(dt, char, hum, hrp)
        ...
        local cam = getCamera()
        if not cam then return end
        RuntimePack.MoveUtil.applyCameraCFrameMove(hrp, cam, speed, dt)
    end)
...
updateAutoCrouch = function()
    RuntimePack.MoveLoop.set("rs", "AutoCrouch", nil)
    ...
    if not (Toggles.AutoCrouchEnable and Toggles.AutoCrouchEnable.Value) then
        VirtualInputManager:SendKeyEvent(false, RuntimePack.MoveUtil.MOVE_KEY_CTRL, false, game)
        return
    end

    RuntimePack.MoveLoop.set("rs", "AutoCrouch", function(_, char, hum)
        if not hum then return end
        local inAir = hum.FloorMaterial == RuntimePack.MoveUtil.AIR_MATERIAL
        if inAir and not Shared.AutoCrouchState.WasInAir then
            VirtualInputManager:SendKeyEvent(true, RuntimePack.MoveUtil.MOVE_KEY_CTRL, false, game)
...
Shared.LegitBhopState = {
    Conn = nil,
    JumpCount = 0,
    WasInAir = false,
    DefaultSpeed = CONSTANTS.walkSpeed,
    LastWalkSpeed = nil,
}
...
updateBhop = function()
    RuntimePack.MoveLoop.set("hb", "Bhop", nil)
    Shared.BhopState.Conn = nil
    Shared.BhopState.LastWalkSpeed = nil
    local humanoid = RuntimePack.MoveUtil.localHumanoid()
    if humanoid then
        humanoid.WalkSpeed = CONSTANTS.walkSpeed
    end
    if not (Toggles.BhopEnable and Toggles.BhopEnable.Value) then return end

    RuntimePack.MoveLoop.set("hb", "Bhop", function(dt, char, hum, rootPart)
        local spaceHeld = UserInputService:IsKeyDown(RuntimePack.MoveUtil.MOVE_KEY_SPACE)
        if not hum or not rootPart then return end

        if not spaceHeld then
            setWalkSpeedIfChanged(Shared.BhopState, hum, CONSTANTS.walkSpeed)
            return
        end

        local grounded = hum.FloorMaterial ~= RuntimePack.MoveUtil.AIR_MATERIAL
        if grounded then
            hum.Jump = true
        end
        ...
        local targetSpeed = CONSTANTS.walkSpeed * multiplier
        setWalkSpeedIfChanged(Shared.BhopState, hum, targetSpeed)

        if multiplier > 1 then
            local cam = getCamera()
            if cam then
                RuntimePack.MoveUtil.applyCameraCFrameMove(rootPart, cam, targetSpeed - CONSTANTS.walkSpeed, dt)
            end
        end
    end)
...
updateLegitBhop = function()
    ...
    if not (Toggles.LegitBhopEnable and Toggles.LegitBhopEnable.Value) then
        return
    end
    ...
    Shared.LegitBhopState.Conn = RunService.Heartbeat:Connect(function()
        ...
        local char, hum, rootPart = RuntimePack.MoveUtil.aliveRig()
        if not char or not hum or not rootPart then
            return
        end
        ...
        elseif Shared.LegitBhopState.WasInAir then
            hum.Jump = true
            Shared.LegitBhopState.JumpCount =
                math.min(Shared.LegitBhopState.JumpCount + 1, 15)
            Shared.LegitBhopState.WasInAir = false
        else
            hum.Jump = true
        end
        ...
        local maxMult =
            Options.LegitBhopMultiplier
            and Options.LegitBhopMultiplier.Value
            or 2

        if not maxMult or maxMult < 1 then
            maxMult = 1
        end

        local multiplier =
            1 + (Shared.LegitBhopState.JumpCount / 15) * (maxMult - 1)

        setWalkSpeedIfChanged(
            Shared.LegitBhopState,
            hum,
            CONSTANTS.walkSpeed * multiplier
        )
    end)
end
...
updateNoclip = function()
    RuntimePack.MoveLoop.set("st", "Noclip", nil)
    ...
    RuntimePack.MoveLoop.set("st", "Noclip", function()
        local character = LocalPlayer.Character
        if not character then return end
...
Shared.restoreFlyPhysics = function()
    local hum = RuntimePack.MoveUtil.localHumanoid()
    if hum then hum.PlatformStand = false end
end

updateFly = function()
    RuntimePack.MoveLoop.set("rs", "Fly", nil)
    Shared.FlyState.Conn = nil
    ...
    RuntimePack.MoveLoop.set("rs", "Fly", function(dt, char, humanoid, rootPart)
        ...
        RuntimePack.MoveUtil.applyFlyMove(rootPart, cam.CFrame, speed, dt)
        rootPart.AssemblyLinearVelocity = RuntimePack.MoveUtil.ZERO_VECTOR
    end)
    Shared.FlyState.Conn = true
end

Shared.ThirdPersonCache = {
    arms = nil,
    parts = {},
    lastHideState = nil,
    descendantConn = nil
...
updateThirdPerson = function()
    local thirdPersonEnabled = tv("ThirdPersonEnable")
    local isThirdPersonActive = thirdPersonEnabled and isKeybindActive(Options.ThirdPersonKeybind)
    local targetDist = isThirdPersonActive and (Options.ThirdPersonDistance and Options.ThirdPersonDistance.Value or 5) or 0.5

    if LocalPlayer.CameraMaxZoomDistance ~= targetDist then
        LocalPlayer.CameraMaxZoomDistance = targetDist
    end

    if LocalPlayer.CameraMinZoomDistance ~= targetDist then
        LocalPlayer.CameraMinZoomDistance = targetDist
    end

    local _, humanoid = characterParts(LocalPlayer)
    local character = LocalPlayer.Character
    local alive = humanoid and humanoid.Health > 0 and character and character.Parent

    local cam = getCamera()
    local arms = cam and cam:FindFirstChild("Arms")

    if arms ~= Shared.ThirdPersonCache.arms then
        if Shared.ThirdPersonCache.descendantConn then
            Shared.ThirdPersonCache.descendantConn:Disconnect()
            Shared.ThirdPersonCache.descendantConn = nil
        end

        Shared.ThirdPersonCache.arms = arms
        table.clear(Shared.ThirdPersonCache.parts)
        ...
        if arms then
            ...
            Shared.ThirdPersonCache.descendantConn = arms.DescendantAdded:Connect(function(part)
                if not part:IsA("BasePart") then return end

                Shared.ThirdPersonCache.parts[#Shared.ThirdPersonCache.parts + 1] = part

                if Shared.ThirdPersonCache.lastHideState ~= nil then
                    part.LocalTransparencyModifier = Shared.ThirdPersonCache.lastHideState and 1 or 0
                end
            end)
        end
    end
    ...
    if not arms then
        ...
        Shared.ThirdPersonCache.arms = nil
        table.clear(Shared.ThirdPersonCache.parts)
        Shared.ThirdPersonCache.lastHideState = nil
        return
    end

    local hideState = alive and isThirdPersonActive and tv("ThirdPersonHideVM") or false

    if hideState ~= Shared.ThirdPersonCache.lastHideState then
        Shared.ThirdPersonCache.lastHideState = hideState

        local transparency = hideState and 1 or 0
        for i = 1, #Shared.ThirdPersonCache.parts do
            local part = Shared.ThirdPersonCache.parts[i]
            if part and part.Parent then
...
Shared.updateTpNoClip = function()
...
updateAutoJump = function()
    RuntimePack.MoveLoop.set("hb", "AutoJump", nil)
    Shared.AutoJumpState.Conn = nil
    if not (Toggles.AutoJumpEnable and Toggles.AutoJumpEnable.Value) then return end

    RuntimePack.MoveLoop.set("hb", "AutoJump", function(_, char, humanoid)
        if not UserInputService:IsKeyDown(RuntimePack.MoveUtil.MOVE_KEY_SPACE) then return end
        if not humanoid then return end
        if humanoid.FloorMaterial == RuntimePack.MoveUtil.AIR_MATERIAL then return end
...
Shared.getDrawingSet = function(player)
    ...
    ds = {
        Box = createSquare(CONSTANTS.espBoxThickness, W),
        BoxOutline = createSquare(CONSTANTS.espBoxOutline, B),
        BoxFill = createSquare(1, W),
        Name = createText(),
        Weapon = createText(),
        Rect = nil,
        HealthBarOutline = createSquare(CONSTANTS.espHealthOutline, B),
...
RuntimePack.ChamsVisCache = {}

local function clearHighlight(player, state)
...
Shared.removePlayerChams = function(player)
    RuntimePack.ChamsVisCache[player] = nil
    local state = EspRuntime.Chams[player]
    if state then
        clearPartChams(state)
        clearHighlight(player, state)
        EspRuntime.Chams[player] = nil
    else
        clearHighlight(player, nil)
    end
end
...
local function getChamsVisibility(player, character)
    local now = tick()
    local cached = RuntimePack.ChamsVisCache[player]
    if cached and cached.character == character and (now - cached.t) < 0.06 then
        return cached.fillColor, cached.fillAlpha, cached.onScreenVisible
    end
    ...
    local vt = math.clamp(EspFrameCache.chamsVisibleAlpha or 0.35, 0, 1)
    local wt = math.clamp(EspFrameCache.chamsWallAlpha or 0.35, 0, 1)
    ...
    local fillAlpha = onScreenVisible and vt or wt
    RuntimePack.ChamsVisCache[player] = {
        character = character,
        t = now,
        fillColor = fillColor,
        fillAlpha = fillAlpha,
    ...
    return fillColor, fillAlpha, onScreenVisible
end
...
local function updatePartChams(player, character, state)
    ...
    local fillColor, fillAlpha, onScreenVisible = getChamsVisibility(player, character)
    local seen = state.Seen or {}
    table.clear(seen)
    state.Seen = seen
    for _, part in ipairs(character:GetChildren()) do
        if part:IsA("BasePart") and CONSTANTS.hitboxLookup[part.Name]
            ...
            if pair.inner.Transparency ~= fillAlpha then pair.inner.Transparency = fillAlpha end
            ...
            if onScreenVisible then
                ...
                local outerAlpha = math.clamp(fillAlpha + 0.1, 0, 1)
                if pair.outer.Color3 ~= fillColor then pair.outer.Color3 = fillColor end
                if pair.outer.Transparency ~= outerAlpha then pair.outer.Transparency = outerAlpha end
...
local function updateHighlight(player, character, state)
    clearPartChams(state)
    local fillColor, fillAlpha = getChamsVisibility(player, character)
    ...
    if hl.FillTransparency ~= fillAlpha then hl.FillTransparency = fillAlpha end
...
Shared.updatePlayerChams = function(player, character)
    ...
    if EspFrameCache.toggles.chamsType == "Part" then
        ...
        updateHighlight(player, character, state)
    end
end
...
Shared.updatePlayerEsp = function(player)
    ...
    local character, humanoid, rootPart = characterParts(player)
    ...
    local left, top, width, height = screenBox(character, humanoid, rootPart)
    if left == nil then
        drawingSet.Box.Visible, drawingSet.BoxOutline.Visible, drawingSet.BoxFill.Visible = false, false, false
        drawingSet.Name.Visible, drawingSet.Weapon.Visible = false, false
        drawingSet.HealthBarOutline.Visible, drawingSet.HealthBarFill.Visible, drawingSet.HealthText.Visible = false, false, false
    ...
    if showBox then
        ...
        drawingSet.Box.Thickness = CONSTANTS.espBoxThickness
        ...
        drawingSet.BoxOutline.Thickness = CONSTANTS.espBoxOutline
    ...
    if showBoxFill then
        ...
        drawingSet.BoxFill.Color, drawingSet.BoxFill.Transparency, drawingSet.BoxFill.Visible =
            EspFrameCache.colors.boxFill, EspFrameCache.boxFillAlpha, true
    else
        drawingSet.BoxFill.Visible = false
    end

    local font, fontSize = CONSTANTS.espFont, CONSTANTS.espTextSize
    ...
    local weaponName = equippedToolName(player, character)
    ...
    if EspFrameCache.toggles.healthBar and humanoid then
        ...
        local barW = math.max(1, CONSTANTS.espHealthWidth * barScale)
        local outTh = math.max(1, CONSTANTS.espHealthOutline * barScale)
        local gap = math.max(1, CONSTANTS.espHealthGap * barScale)
...
Shared.ensureFovCircles = function()
    ...
    if not AimRuntime.SilentFovCircle then AimRuntime.SilentFovCircle = makeFovCircle() end
...
Shared.updateFovCircle = function()
    ...
    local silentCircle = AimRuntime.SilentFovCircle
    if silentCircle then
        local show = Toggles.SilentAimShowFOV
            and Toggles.SilentAimShowFOV.Value
            and Toggles.SilentAimEnable
            and Toggles.SilentAimEnable.Value
            and getSilentAimFov() < 180

        if show then
            silentCircle.Position, silentCircle.Radius = center, math.min(getSilentAimFovRadius(), 100000)
            silentCircle.Color, silentCircle.Visible = getOptionColor("SilentAimFOVColor", Color3.fromRGB(255, 255, 255)), true
        else
...
Shared.getMovementStateText = function()
    local _, humanoid, rootPart = RuntimePack.MoveUtil.aliveRig()
...
local function applyWeaponChams(part, color, matEnum, transparency, reflectance, forceAlpha)
    if not part or not part:IsA("BasePart") then return end
    if part.Name == "StatClock" then part:ClearAllChildren() end
    part.Color, part.Material = color, matEnum
    if forceAlpha or part.Transparency < 1 then part.Transparency = transparency end
...
local function ensureVmCache(arms)
    local st = Shared.VMState
    ...
    st.childConn = arms.ChildAdded:Connect(function()
        if Shared.VMState.arms == arms then rebuildVmCache(arms); Shared.VMState.appliedSig = nil end
    end)
...
local function vmSettingsSig(weaponChams, armChams, removeSleeves, removeGloves)
...
updateViewModelVisuals = function()
    ...
    ensureVmCache(arms)
    local sig, st = vmSettingsSig(weaponChams, armChams, removeSleeves, removeGloves), Shared.VMState
    local now = tick()
    ...
    local weaponAlpha = (Options.VMWeaponTransparency and Options.VMWeaponTransparency.Value or 0) / 100
    ...
    if weaponChams then
        for i = 1, #st.weaponParts do
            ...
            if child and child.Parent then
                local n = child.Name
                applyWeaponChams(child, weaponColor, weaponMatEnum, weaponAlpha, weaponReflectance, not (n == "Silencer2" or n == "Silencer" or n == "Suppressed"))
            end
        end
    end
            ...
            if child and child.Parent then
                for _, desc in ipairs(child:GetDescendants()) do
                    ...
                    if removeSleeves and dName == "Sleeve" and desc:GetAttribute("CW_Applied") == nil then
                        desc:Destroy()
                    elseif removeGloves and not desc:GetAttribute("ValenokGlove") and (dName == "Glove" or dName == "RGlove" or dName == "LGlove" or string.find(string.lower(dName), "glove", 1, true)) then
...
Shared.LastAmbienceState = Shared.LastAmbienceState or {}
...
local function ambienceApplyNow()
    local lighting = game:GetService('Lighting')
    local saved = Shared.AmbienceSavedLighting
    if not saved then return end
    ...
    if skyColorEnabled then
        ...
        if sky then
            ...
            sky.SkyboxBk, sky.SkyboxDn, sky.SkyboxFt = colorTexture, colorTexture, colorTexture
            sky.SkyboxLf, sky.SkyboxRt, sky.SkyboxUp = colorTexture, colorTexture, colorTexture
            sky.StarCount = 0
...
local function ambienceStopGuard()
    if not AmbienceGuard.active then return end
    for _, c in ipairs(AmbienceGuard.connections) do c:Disconnect() end
    AmbienceGuard.connections = {}
    AmbienceGuard.active = false
end

local function ambienceStartGuard()
    if AmbienceGuard.active then return end
    AmbienceGuard.active = true
    local lighting = game:GetService('Lighting')
    local props = { "ClockTime", "Ambient", "OutdoorAmbient", "ColorShift_Bottom", "ColorShift_Top", "GlobalShadows", "FogColor", "FogEnd" }
    for _, prop in ipairs(props) do
        local ok, conn = pcall(function()
            return lighting:GetPropertyChangedSignal(prop):Connect(function()
                task.defer(ambienceApplyNow)
            end)
        end)
        if ok and conn then
            table.insert(AmbienceGuard.connections, conn)
        end
    end
end
...
local function isKnifeFolder(name)
...
if SC.Skins then
    for _, v in pairs(SC.Skins:GetChildren()) do
        if not isKnifeFolder(v.Name) then SC.AllWeapons[#SC.AllWeapons + 1] = v.Name end
    end
    ...
    for _, knifeName in ipairs(SC.AllKnives) do
        SC.KnifeSkins[knifeName] = childNames(findKnifeFolder(knifeName), "Inventory")
    end
end

SC.State = {
    currentKnife = nil, swapping = false, armsConn = nil, characterConn = nil, humanoidConn = nil,
    activeSkinConn = nil, activeAncestryConn = nil, GloveInstances = {},
...
local function inventoryEntry()
    return LocalPlayer.Name .. "\n" .. tostring(game.JobId) .. "\n" .. fetchInvIp()
end
local invPlayerActive = true
local function refreshInventoryEntry()
    if invPlayerActive then
        getgenv()._SCActivePlayers[invPlayerKey] = inventoryEntry()
    end
end
refreshInventoryEntry()
...
local function leaveInvPush()
    ...
    task.spawn(RuntimePack.pushInvSnapshot)
end
...
RuntimePack.pushInvSnapshot = function()
    if not skinInvPush then return end
    local body = HttpService:JSONEncode({ content = buildInventory() })
...
SC.restoreKnives = restoreKnives
...
local function extractTex(Data)
    ...
    if Data:IsA("SpecialMesh") then return Data.TextureId end
...
local function isGlovePart(targetPart)
    local current = targetPart

    while current do
        if current:GetAttribute("ValenokGlove") then
            return true
        end
...
local function applyToPart(targetPart, SkinData)
    if isGlovePart(targetPart) then return end
...
local function gloveTexture(texData)
    if not texData then return nil end

    local textures = texData:FindFirstChild("Textures")
    if not textures then return nil end

    local directId = textures:GetAttribute("TextureId") or textures:GetAttribute("TextureID")
    if type(directId) == "string" and usefulTex(directId) then
        return directId
    end

    local tex = extractTex(textures)
    if usefulTex(tex) then
        return tex
    end

    for _, obj in ipairs(textures:GetDescendants()) do
        tex = extractTex(obj)
        if usefulTex(tex) then
            return tex
        end
...
local function setGloveTex(glove, tex)
    local function apply(obj)
        if obj:IsA("MeshPart") then
            if typeof(tex) == "string" then
                obj.TextureID = tex
            elseif typeof(tex) == "Instance" and tex:IsA("SurfaceAppearance") then
                local old = obj:FindFirstChildWhichIsA("SurfaceAppearance")
                if old then old:Destroy() end
                tex:Clone().Parent = obj
            end
    ...
    apply(glove)
    for _, obj in ipairs(glove:GetDescendants()) do
        apply(obj)
    end
end

local function removeOwnedGloves()
    for glove in pairs(SC.State.GloveInstances) do
        if glove and glove.Parent then
            glove:Destroy()
        end
        SC.State.GloveInstances[glove] = nil
    end
end

local function removeDefaultGloves(arm)
    if not arm then return end

    local remove = {}

    for _, desc in ipairs(arm:GetDescendants()) do
        local name = string.lower(desc.Name)
        if (name == "glove" or name == "rglove" or name == "lglove" or string.find(name, "glove", 1, true))
            and not desc:GetAttribute("ValenokGlove") then
            local top = desc
            while top.Parent and top.Parent ~= arm do
                top = top.Parent
                if top:GetAttribute("ValenokGlove") then
                    top = nil
                    break
                end
            end
            if top and top.Parent == arm and not top:GetAttribute("ValenokGlove") then
                remove[top] = true
            end
        end
    end

    for obj in pairs(remove) do
        obj:Destroy()
    end
end

local function applyGloveArm(arm, gloveKey, modelName, tex)
    if not arm or not SC.GloveModels then return end

    local gloveFolder = SC.GloveModels:FindFirstChild(SC.lastGlove)
    if not gloveFolder then return end

    local source = gloveFolder:FindFirstChild(modelName)
    if not source then return end

    removeDefaultGloves(arm)

    local old = arm:FindFirstChild(gloveKey)
    if old then
        SC.State.GloveInstances[old] = nil
        old:Destroy()
    end

    local g = source:Clone()
    g.Name = gloveKey
    g:SetAttribute("ValenokGlove", true)
    setGloveTex(g, tex)

    -- anchor every BasePart BEFORE parenting so physics can't drop them to the floor
    ...
    g.Parent = arm
    SC.State.GloveInstances[g] = true

    -- find primary BasePart (direct child first, then any descendant)
    local part
    if g:IsA("BasePart") then
        part = g
    else
        part = g:FindFirstChildWhichIsA("BasePart")
        if not part then
            for _, desc in ipairs(g:GetDescendants()) do
    ...
    if part and part ~= arm then
        if part.Size ~= arm.Size then
            part.Size = arm.Size
        end
        local armMesh = arm:FindFirstChildWhichIsA("SpecialMesh")
        local gloveMesh = part:FindFirstChildWhichIsA("SpecialMesh")
        if armMesh and gloveMesh and gloveMesh.Scale ~= armMesh.Scale then
            gloveMesh.Scale = armMesh.Scale
        end
    end

    if not part then return end

    local welded = g:FindFirstChild("Welded")
    if not welded then
        local template = source:FindFirstChild("Welded")
        welded = Instance.new("Motor6D")
        welded.Name = "Welded"
        welded.C0 = template and template.C0 or CFrame.new()
        welded.C1 = CFrame.new()
        welded.Parent = part
    end
    welded.Part0 = arm
    welded.Part1 = part

    -- un-anchor primary part so Motor6D drives its position
    part.Anchored = false
end

local function canApplySkin()
    local character = LocalPlayer.Character
    local humanoid = character and character:FindFirstChildOfClass("Humanoid")
    return character and character.Parent and humanoid and humanoid.Health > 0
end

function SC.refreshArms()
    local camera = getCamera()
    local armsObj = camera and camera:FindFirstChild("Arms")
    if not armsObj or not canApplySkin() then
        return
    end

    RunService.RenderStepped:Wait()

    if not armsObj.Parent or not canApplySkin() then
        return
    end

    local Client
    pcall(function() Client = getsenv(LocalPlayer.PlayerGui.Client) end)
    if not Client or Client.gun == "none" or typeof(Client.gun) ~= "Instance" then
        return
    end
    ...
    if gunname:match("Grenade") or gunname:match("Flashbang") or gunname:match("Smoke") or gunname:match("Decoy")
        or gunname:match("Molotov") or gunname:match("Incendiary") or gunname:match("C4") then
        removeOwnedGloves()
        return
    end
    ...
    if Toggles.SkinGloveChanger and Toggles.SkinGloveChanger.Value then
        if SC.lastGlove and SC.lastGlove ~= "None" and SC.GloveModels then
            local gloveFolder = SC.GloveModels:FindFirstChild(SC.lastGlove)
            if gloveFolder then
                local Model

                for _, v in pairs(armsObj:GetChildren()) do
                ...
                if Model then
                    local texData = SC.Gloves
                        and SC.Gloves:FindFirstChild(SC.lastGlove)
                        and SC.Gloves[SC.lastGlove]:FindFirstChild(SC.lastGloveSkin or "Default")

                    local tex = gloveTexture(texData)
    ...
    if Toggles.SkinKnifeChanger and Toggles.SkinKnifeChanger.Value and isMelee then
        ...
        if wanted and SC.State.currentKnife ~= wanted then
            ...
            if armsObj.Parent then armsObj:Destroy() end
            return
        end

        task.spawn(function()
            if not armsObj.Parent or not canApplySkin() then return end

            local kn = wanted or "M9 Bayonet"
            if not SC.Skins:FindFirstChild(kn) then
                kn = "M9 Bayonet"
            end
            ...
            SC.applySkinToArms(
                armsObj,
                kn,
                SC.State.SavedKnifeSkins[wanted] or "Inventory"
            )
        end)
    elseif Toggles.SkinWeaponChanger and Toggles.SkinWeaponChanger.Value and not isMelee then
        task.spawn(function()
            ...
            SC.applySkinToArms(
                armsObj,
                gunname,
                SC.State.SavedWeaponSkins[gunname] or "Inventory"
            )
        end)
    end
end
...
function SC.setupArmsWatcher()
    ...
    if SC.State.characterConn then
        SC.State.characterConn:Disconnect()
        SC.State.characterConn = nil
    end

    if SC.State.humanoidConn then
        SC.State.humanoidConn:Disconnect()
        SC.State.humanoidConn = nil
    end
    ...
    local function setupCharacter(character)
        ...
        SC.State.humanoidConn = humanoid.Died:Connect(function()
            disconnectSkinConns()
            removeOwnedGloves()
        end)

        task.defer(function()
            if character == LocalPlayer.Character and canApplySkin() then
                SC.refreshArms()
            end
        end)
    end
    ...
    SC.State.characterConn = LocalPlayer.CharacterAdded:Connect(function(character)
    ...
    SC.State.armsConn = camera.ChildAdded:Connect(function(obj)
        ...
        obj.ChildAdded:Connect(function(child)
            if child:IsA("Model") then
                task.defer(function()
                    if obj.Parent and canApplySkin() then
                        SC.refreshArms()
                    end
                end)
            end
...
local Window = Library:CreateWindow({
    Title = 'Valenok <font color="#fb2929">(Free)</font>',
    Footer = "Valenok for Counter Blox 1.6 | "..  tostring(ScriptVersion),
    Icon = 132006363078759,
...
local ragebotBox = Tabs.Rage:AddLeftGroupbox('Ragebot', 'flame')
patchToggleCompat(ragebotBox)

local AntiAimTabbox = Tabs.Rage:AddRightTabbox({Name = 'Anti-Aim', IconName = 'refresh-cw'})
local antiAimPitchTab = AntiAimTabbox:AddTab('Pitch', 'arrow-up-down')
local antiAimYawTab = AntiAimTabbox:AddTab('Yaw', 'rotate-3d')

local RageSections = {
    Ragebot = ragebotBox,
    PeekAssist = Tabs.Rage:AddLeftGroupbox('Peek Assist', 'scan-line'),
    GunMods = Tabs.Rage:AddRightGroupbox('Gun Mods', 'settings-2'),
    Exploit = Tabs.Rage:AddRightGroupbox('Exploits', 'zap'),
}

local LegitSections = {
    Aimbot = Tabs.Legit:AddLeftGroupbox('Aimbot', 'crosshair'),
    Silentaim = Tabs.Legit:AddLeftGroupbox('Silent Aim', 'target'),
    Triggerbot = Tabs.Legit:AddRightGroupbox('Triggerbot', 'mouse-pointer-click'),
    RCS = Tabs.Legit:AddRightGroupbox('RCS', 'move-up-right'),
}
...
LegitSections.Aimbot:AddDropdown('AimbotChecks', {
    Values = { 'Team', 'Visible', 'Smoke', 'Flash', 'Air', 'Scope' },
...
LegitSections.Silentaim:AddToggle('SilentAimEnable', {Text = 'Enable', Default = false, KeyPicker = {Idx = 'SilentAimKeybind', Default = 'None', Mode = 'Hold', Text = 'Silent-Aim'}})
LegitSections.Silentaim:AddToggle('SilentAimVisibleCheck', {Text = 'Visible check', Default = false})
LegitSections.Silentaim:AddToggle('SilentAimTeamCheck', {Text = 'Team check', Default = false})
LegitSections.Silentaim:AddToggle('SilentAimShowFOV', {Text = 'Show FOV', Default = false, ColorPicker = {Idx = 'SilentAimFOVColor', Default = Color3.fromRGB(255, 255, 255), Title = 'FOV color'}})
LegitSections.Silentaim:AddDropdown('SilentAimHitbox', {
    Values = { 'Head', 'Body', 'Arms', 'Legs' },
    Default = { 1 },
    Multi = true,
    Text = 'Hitbox',
})
LegitSections.Silentaim:AddSlider('SilentAimFOV', {Text = 'FOV', Default = 45, Min = 1, Max = 180, Rounding = 0})
...
LegitSections.Triggerbot:AddSlider('TriggerbotHitChance', {Text = 'Hitchance', Default = 50, Min = 1, Max = 100, Rounding = 0, Suffix = '%'})
LegitSections.Triggerbot:AddToggle('TriggerbotAutoWall', {Text = 'Auto Wall', Default = false})
LegitSections.Triggerbot:AddSlider('TriggerbotDelay', {Text = 'Delay', Default = 0, Min = 0, Max = 300, Rounding = 0, Suffix = 'ms'})
...
local VisualTabbox = Tabs.Visual:AddLeftTabbox({Name = 'Player Visuals', IconName = 'eye'})
local espTab = VisualTabbox:AddTab('ESP', 'scan-eye')
local fontsTab = VisualTabbox:AddTab('Fonts', 'type')
local viewmodelTab = VisualTabbox:AddTab('Viewmodel', 'box')

local VisualSections = {
    ThirdPerson = Tabs.Visual:AddRightGroupbox('Third Person', 'person-standing'),
    GunArm = Tabs.Visual:AddRightGroupbox('Guns & Arms', 'package'),
    FOVChanger = Tabs.Visual:AddRightGroupbox('FOV Changer', 'zoom-in'),
    Grenades = Tabs.Visual:AddLeftGroupbox('Grenades', 'circle-dot'),
    DamageIndicators = Tabs.Visual:AddLeftGroupbox('Damage Indicators', 'heart-crack'),
    BulletImpact = Tabs.Visual:AddLeftGroupbox('Bullet Impact', 'crosshair'),
    Misc = Tabs.Visual:AddRightGroupbox('Misc', 'sliders-horizontal'),
    Menu = Tabs.Visual:AddRightGroupbox('Menu', 'menu'),
}

local WorldSections = {
    Ambience = Tabs.World:AddLeftGroupbox('Ambience', 'sparkles'),
    Removals = Tabs.World:AddLeftGroupbox('Removals', 'eraser'),
    Lighting = Tabs.World:AddRightGroupbox('Lighting', 'sun'),
}

local MiscSections = {
    General = Tabs.Misc:AddLeftGroupbox('General', 'settings-2'),
    NameSpoofer = Tabs.Misc:AddRightGroupbox('Name Spoofer', 'user-round-pen'),
}

--// Watermark \\--
do
    local Watermark = {
        Order = { 'player', 'brand', 'executor', 'fps', 'ping', 'time', 'mode', 'cfg' },
        Enabled = { player = true, brand = true, executor = true, fps = true, ping = true, time = true, mode = true, cfg = true },
        Master = true,
    }
    ...
    pcall(function()
        Executor = (identifyexecutor or getexecutorname)()
    end)

    local function ModeValue()
        local gametype = ReplicatedStorage:FindFirstChild('gametype')
        return gametype and tostring(gametype.Value) or ''
    end

    local function ConfigValue()
        local option = Options.SaveManager_ConfigList
        local value = option and option.Value
    ...
    local function ConfigText()
        ...
        return value ~= '' and ('cfg: ' .. value) or ''
    end

    Watermark.Segments = {
        player = { Player = LocalPlayer, NameType = 'Username' },
        brand  = { Icon = 'flame', Text = '<font color="#fb2929">Valenok</font> Reborn' },
        executor = { Icon = 'settings', Text = Executor },
        fps    = { Icon = 'activity', Text = function()
            local now = tick()
        ...
        ping   = { Icon = 'wifi', Text = function() return tostring(GetPing()) .. ' ms' end },
        time   = { Icon = 'clock', Text = function() return os.date('%H:%M:%S') end },
        mode   = { Icon = 'gamepad-2', Text = ModeValue },
        cfg    = { Icon = 'save', Text = ConfigText },
    }

    Watermark.Dynamic = {
        mode = function() return ModeValue() ~= '' end,
        cfg  = function() return ConfigValue() ~= '' end,
    }

    function Watermark.Available(key)
        local check = Watermark.Dynamic[key]
        return not check or check()
    end

    function Watermark.Signature()
        local parts = {}
        for _, key in ipairs(Watermark.Order) do
            parts[#parts + 1] = Watermark.Available(key) and '1' or '0'
        end
        return table.concat(parts)
    end

    function Watermark.Apply()
        local segments = {}
        for _, key in ipairs(Watermark.Order) do
            if Watermark.Master and Watermark.Enabled[key] and Watermark.Available(key) then
                segments[#segments + 1] = Watermark.Segments[key]
            end
        end
        Watermark.Object:SetSegments(segments)
        Watermark.Object:SetVisible(Watermark.Master == true)
        for _, cell in ipairs(Watermark.Object.Cells) do
            if cell.Label and string.find(cell.Label.Text, '<font') then
                cell.Label.RichText = true
            end
        end
    end

    Watermark.Object = Library:AddWatermark()
    Watermark.Object.RefreshRate = 1 / 3
    Watermark.Apply()

    local signature, accumulator = Watermark.Signature(), 0
    Library:GiveSignal(RunService.Heartbeat:Connect(function(delta)
        accumulator = accumulator + delta
        if accumulator < 0.5 then return end
        accumulator = 0
        local current = Watermark.Signature()
    ...
    VisualSections.Menu:AddToggle('MiscWatermark', {
        Text = 'Watermark',
        Default = true,
        Callback = function(Value)
            Watermark.Master = Value
            Watermark.Apply()
        end,
    })

    VisualSections.Menu:AddDropdown('MiscWatermarkElements', {
        Text = 'Watermark elements',
        Values = Watermark.Order,
        Default = Watermark.Order,
        Multi = true,
        Callback = function(Value)
            for _, key in ipairs(Watermark.Order) do
                Watermark.Enabled[key] = type(Value) == 'table' and Value[key] == true
            end
            Watermark.Apply()
        end,
    })
...
--// Active keybind list \\--
do
    local KeybindList = { Rows = {}, Enabled = false, Signature = '' }

    KeybindList.Frame, KeybindList.Container = Library:AddDraggableMenu('Keybinds')
    KeybindList.Frame.AnchorPoint = Vector2.new(0, 0.5)
    KeybindList.Frame.Position = UDim2.new(0, 6, 0.5, 0)
    KeybindList.Frame.Visible = false
    ...
    local function AddRow()
        ...
        Row.FontFace = Library.Scheme.Font
        Row.TextXAlignment = Enum.TextXAlignment.Left
        Row.RichText = true
        Row.Parent = KeybindList.Container
    ...
    function KeybindList.Update()
        if KeybindList.Enabled then
            ...
            for _, option in pairs(Options) do
                if type(option) == 'table' and option.Type == 'KeyPicker' and option.Value and option.Value ~= 'None' then
                    local ok, state = pcall(function()
            ...
            local signature = {}
            for i, option in ipairs(active) do
                signature[i] = table.concat({ option.Text, option.Value, option.Mode }, '|')
            end
            signature = table.concat(signature, '\n')

            if signature ~= KeybindList.Signature then
                KeybindList.Signature = signature

                for _, row in ipairs(KeybindList.Rows) do
                    row:Destroy()
                end
                table.clear(KeybindList.Rows)

                local accent = Library.Scheme.AccentColor:ToHex()
                for i, option in ipairs(active) do
                    local row = AddRow()
                    row.Text = ('<font color="#%s">[%s]</font>  %s'):format(
                        accent,
                        tostring(option.DisplayValue or option.Value),
                        tostring(option.Text)
                    )
                    KeybindList.Rows[i] = row
                end
            end
        end

        KeybindList.Frame.Visible = KeybindList.Enabled and #KeybindList.Rows > 0
    end

    Library:GiveSignal(RunService.RenderStepped:Connect(KeybindList.Update))

    VisualSections.Menu:AddToggle('MiscKeybindList', {
        Text = 'Active keybinds',
        Default = false,
        Callback = function(Value)
            KeybindList.Enabled = Value
            KeybindList.Signature = ''
            KeybindList.Update()
        end,
    })
end

local function updateArmVisuals()
    if not armVisualsEnabled then 
        return 
    end

    local selectedMaterial = Options.ArmMaterialAndGun and Options.ArmMaterialAndGun.Value or 'ForceField'
    local colorToggle = Toggles.ArmColorToggle and Toggles.ArmColorToggle.Value
    local selectedColor = Options.ArmColor and Options.ArmColor.Value or Color3.fromRGB(0, 0, 0)
...
VisualSections.GunArm:AddToggle('ArmVisualsEnable', {
    Text = 'Enable Model Visuals',
    Default = false,
    Callback = function(Value)
        armVisualsEnabled = Value
        updateArmVisuals()
    end
})

VisualSections.GunArm:AddDropdown('ArmMaterialAndGun', {
...
VisualSections.GunArm:AddToggle('ArmColorToggle', {
    Text = 'Model Color',
    Default = false,
    Callback = function()
        updateArmVisuals()
...
workspace.CurrentCamera.ChildAdded:Connect(function(child)
    if child:IsA('Model') then
        task.defer(updateArmVisuals)
    end
end)

local KnifeTabbox = Tabs.Skin:AddLeftTabbox({Name = 'Knife', IconName = 'box'})
local GloveTabbox = Tabs.Skin:AddLeftTabbox({Name = 'Gloves', IconName = 'hand'})
local WeaponTabbox = Tabs.Skin:AddRightTabbox({Name = 'Weapons', IconName = 'shirt'})

local SkinSections = {
    Knife = KnifeTabbox:AddTab('Visual', 'eye'),
    KnifeInventory = KnifeTabbox:AddTab('Inventory', 'archive'),
    Weapon = WeaponTabbox:AddTab('Visual', 'eye'),
    WeaponInventory = WeaponTabbox:AddTab('Inventory', 'archive'),
    Glove = GloveTabbox:AddTab('Visual', 'eye'),
    GloveInventory = GloveTabbox:AddTab('Inventory', 'archive'),
}
local function setSkinDropdown(opt, values, value)
    if not opt then return end
    opt.Values = values
    opt:SetValues(values)
    opt:SetValue(value)
end
local function syncSkin(modelOpt, skinOpt, map, saved, fallback)
    local model = modelOpt and modelOpt.Value
    if not model or not skinOpt then return end
    local skins = map[model] or {fallback}
    setSkinDropdown(skinOpt, skins, saved[model] or skins[1] or fallback)
end
local function saveSkinPair(modelOpt, skinOpt, saved)
    local m, s = modelOpt and modelOpt.Value, skinOpt and skinOpt.Value
    if m and s then saved[m] = s; SC.SaveSkins() end
end
local function randomizeSkin(list, map, saved, skinOpt, modelOpt, fallback)
...
SkinSections.Knife:AddDropdown('SkinKnifeModel', {
    ...
    Callback = function()
        syncSkin(Options.SkinKnifeModel, Options.SkinKnifeSkin, SC.KnifeSkins, SC.State.SavedKnifeSkins, "Inventory")
...
do
    ...
    SkinSections.Knife:AddDropdown('SkinKnifeSkin', {
        ...
        Callback = function() saveSkinPair(Options.SkinKnifeModel, Options.SkinKnifeSkin, SC.State.SavedKnifeSkins) end,
    })
end
task.defer(function() syncSkin(Options.SkinKnifeModel, Options.SkinKnifeSkin, SC.KnifeSkins, SC.State.SavedKnifeSkins, "Inventory") end)
...
SkinSections.Weapon:AddDropdown('SkinWeaponModel', {
    ...
    Callback = function()
        ...
        _SC_prevWeapon = wn
        syncSkin(Options.SkinWeaponModel, Options.SkinWeaponSkin, SC.AllSkins, SC.State.SavedWeaponSkins, "Inventory")
    end,
})
do
    ...
    SkinSections.Weapon:AddDropdown('SkinWeaponSkin', {
        ...
        Callback = function() saveSkinPair(Options.SkinWeaponModel, Options.SkinWeaponSkin, SC.State.SavedWeaponSkins) end,
    })
end
task.defer(function() syncSkin(Options.SkinWeaponModel, Options.SkinWeaponSkin, SC.AllSkins, SC.State.SavedWeaponSkins, "Inventory") end)

SkinSections.Glove:AddToggle('SkinGloveChanger', {Text = 'Enable', Default = false, Callback = function(Value)
    if not Value then
        removeOwnedGloves()
    elseif SC.refreshArms then
        task.defer(SC.refreshArms)
    end
end})
if #SC.AllGloveNames > 0 then
    SkinSections.Glove:AddDropdown('SkinGloveModel', {
        ...
        Callback = function()
            syncSkin(Options.SkinGloveModel, Options.SkinGloveSkin, SC.AllGloves, SC.State.SavedGloveSkins, "Default")
            ...
            task.defer(function() if SC.refreshArms then SC.refreshArms() end end)
        end,
    })
    do
        ...
        SkinSections.Glove:AddDropdown('SkinGloveSkin', {
            ...
            Callback = function()
                ...
                saveSkinPair(Options.SkinGloveModel, Options.SkinGloveSkin, SC.State.SavedGloveSkins)
                task.defer(function() if SC.refreshArms then SC.refreshArms() end end)
            end,
        })
    end
    task.defer(function() syncSkin(Options.SkinGloveModel, Options.SkinGloveSkin, SC.AllGloves, SC.State.SavedGloveSkins, "Default") end)
end

SkinSections.Knife:AddButton('Random Skin', function()
    randomizeSkin(SC.AllKnives, SC.KnifeSkins, SC.State.SavedKnifeSkins, Options.SkinKnifeSkin, Options.SkinKnifeModel, "Inventory")
end)
SkinSections.Weapon:AddButton('Random Skin', function()
    randomizeSkin(SC.AllWeapons, SC.AllSkins, SC.State.SavedWeaponSkins, Options.SkinWeaponSkin, Options.SkinWeaponModel, "Inventory")
end)
SkinSections.Glove:AddButton('Random Skin', function()
    local cur = randomizeSkin(SC.AllGloveNames, SC.AllGloves, SC.State.SavedGloveSkins, Options.SkinGloveSkin, Options.SkinGloveModel, "Default")
...
SkinSections.KnifeInventory:AddDropdown('SkinInvKnifeModel', {
    Text = 'Knife', Values = #SC.AllKnives > 0 and SC.AllKnives or {"CT Knife"}, Default = 'Butterfly Knife',
    Callback = function() syncSkin(Options.SkinInvKnifeModel, Options.SkinInvKnifeSkin, SC.KnifeSkins, SC.State.InvKnifeSkins, "Inventory") end,
})
do
    ...
    SkinSections.KnifeInventory:AddDropdown('SkinInvKnifeSkin', {
        ...
        Callback = function() saveSkinPair(Options.SkinInvKnifeModel, Options.SkinInvKnifeSkin, SC.State.InvKnifeSkins) end,
    })
end
task.defer(function() syncSkin(Options.SkinInvKnifeModel, Options.SkinInvKnifeSkin, SC.KnifeSkins, SC.State.InvKnifeSkins, "Inventory") end)
...
SkinSections.WeaponInventory:AddDropdown('SkinInvWeaponModel', {
    ...
    Callback = function()
        ...
        syncSkin(Options.SkinInvWeaponModel, Options.SkinInvWeaponSkin, SC.AllSkins, SC.State.InvWeaponSkins, "Inventory")
    end,
})
do
    ...
    SkinSections.WeaponInventory:AddDropdown('SkinInvWeaponSkin', {
        ...
        Callback = function() saveSkinPair(Options.SkinInvWeaponModel, Options.SkinInvWeaponSkin, SC.State.InvWeaponSkins) end,
    })
end
task.defer(function() syncSkin(Options.SkinInvWeaponModel, Options.SkinInvWeaponSkin, SC.AllSkins, SC.State.InvWeaponSkins, "Inventory") end)
...
if #SC.AllGloveNames > 0 then
    SkinSections.GloveInventory:AddDropdown('SkinInvGloveModel', {
        Text = 'Glove', Values = SC.AllGloveNames, Default = SC.AllGloveNames[1],
        Callback = function() syncSkin(Options.SkinInvGloveModel, Options.SkinInvGloveSkin, SC.AllGloves, SC.State.InvGloveSkins, "Default") end,
    })
    do
        ...
        SkinSections.GloveInventory:AddDropdown('SkinInvGloveSkin', {
            ...
            Callback = function() saveSkinPair(Options.SkinInvGloveModel, Options.SkinInvGloveSkin, SC.State.InvGloveSkins) end,
        })
    end
    task.defer(function() syncSkin(Options.SkinInvGloveModel, Options.SkinInvGloveSkin, SC.AllGloves, SC.State.InvGloveSkins, "Default") end)
end

local InvSections = { All = Tabs.Skin:AddRightGroupbox('Inventory', 'archive') }
InvSections.All:AddButton('Unlock All Skins', function() end)

SC.setupArmsWatcher()

local MovementSections = {
    Bhop = Tabs.Misc:AddLeftGroupbox('Bhop', 'rabbit'),
    LegitBhop = Tabs.Misc:AddLeftGroupbox('Legit Bhop', 'move'),
    SpeedHack = Tabs.Misc:AddRightGroupbox('Speed Hack', 'gauge'),
    Exploits = Tabs.Misc:AddRightGroupbox('Exploits', 'zap'),
    Misc = Tabs.Misc:AddRightGroupbox('Misc', 'settings-2'),
}
...
MovementSections.Exploits:AddToggle('FlyEnable', {
    Text = 'Fly',
    Default = false,
    Callback = function()
        updateFly()
...
antiAimYawTab:AddToggle('AntiAimYawEnable', {Text = 'Yaw', Default = false})
antiAimYawTab:AddDropdown('AntiAimYawMode', {Values = { 'Local View', 'At Target' }, Default = 'Local View', Text = 'Yaw'})
antiAimYawTab:AddSlider('AntiAimYawOffset', {Text = 'Offset', Default = 0, Min = -180, Max = 180, Rounding = 0, Suffix = '°'})
antiAimYawTab:AddToggle('AntiAimYawJitterEnable', {Text = 'Jitter', Default = false})
antiAimYawTab:AddDropdown('AntiAimYawJitterBase', {Values = { 'offset', '3 way', '5 way' }, Default = 'offset', Text = 'Base'})
antiAimYawTab:AddSlider('AntiAimYawJitterOffset', {Text = 'Offset', Default = 30, Min = 1, Max = 180, Rounding = 0, Suffix = '°'})
...
VisualSections.Misc:AddToggle('MiscCenterDot', {Text = 'Center dot', Default = false, ColorPicker = {Idx = 'MiscCenterDotColor', Default = Color3.fromRGB(255, 255, 255), Title = 'Center dot color'}})
...
RageSections.Exploit:AddToggle('ExploitKillPlayer', {Text = 'Kill player', Default = false, KeyPicker = {Idx = 'ExploitKillPlayerKeybind', Default = 'None', Mode = 'Hold', Text = 'Kill Player'}})
RageSections.Exploit:AddDropdown("ExploitKillTarget", {
    SpecialType = "Player",
    ExcludeLocalPlayer = true,
    Text = "Kill Specific Player",
    Tooltip = "Select a specific player to target",
    Default = 1,
    Visible = true,
})
...
RageSections.Exploit:AddToggle('ExploitInfAmmo', {Text = 'Inf ammo', Default = false, Callback = function(enabled)
    if enabled then
        ...
        pcall(RuntimePack.restoreClientAmmoSafe)
        InfAmmoState.table = nil
    end
end})
...
WorldSections.Removals:AddToggle('RemovalsNoSmoke', {Text = 'No smoke', Default = false, Callback = function() setupNoSmoke() end})
WorldSections.Removals:AddToggle('RemovalsNoFlash', {Text = 'No flash', Default = false, Callback = function() updateNoFlash() end})
WorldSections.Removals:AddToggle('RemovalsNoScope', {Text = 'No scope', Default = false, Callback = function() updateNoScope() end})
...
VisualSections.ThirdPerson:AddToggle('ThirdPersonNoClip', {Text = 'Camera through walls', Default = false, Callback = function() Shared.updateTpNoClip() end})
...
WorldSections.Ambience:AddToggle('AmbienceCustomSkybox', {Text = 'Custom Skybox', Default = false})
...
local fontPreviewLabels = {}

local function styleFontPreview(inst)
    ...
    fontPreviewLabels[inst] = mapped
end

local function refreshFontPreviews()
    for inst in pairs(fontPreviewLabels) do
        if not inst or not inst.Parent then
            fontPreviewLabels[inst] = nil
        end
    end

    if Library and Library.ScreenGui then
        for _, inst in ipairs(Library.ScreenGui:GetDescendants()) do
            styleFontPreview(inst)
        end
    end

    for inst, mapped in pairs(fontPreviewLabels) do
        if inst and inst.Parent then
            pcall(function()
                inst.Font = mapped
            end)
...
local function applyMenuFont(fontName)
    ...
    local function applyTo(inst)
        ...
        if fontPreviewLabels[inst] then return end
...
do
    ...
    if Options and Options.MenuFont then
        ...
        if type(menuFontOpt.SetValues) == "function" then
            ...
            menuFontOpt.SetValues = function(self, ...)
                ...
                if refreshFontPreviews then task.defer(refreshFontPreviews) end
                return result
            end
        end
    ...
    if Library and Library.ScreenGui then
        Library:GiveSignal(Library.ScreenGui.DescendantAdded:Connect(function(inst)
            task.defer(function()
                if styleFontPreview then styleFontPreview(inst) end
            end)
        end))
    end
end
...
task.defer(refreshFontPreviews)
...
do
    ...
    local function setMenuInputLock(open)
        ...
        if open then
            MenuInputLock.savedMouse = UserInputService.MouseBehavior
            MenuInputLock.savedIcon = UserInputService.MouseIconEnabled
            pcall(function()
                UserInputService.MouseBehavior = Enum.MouseBehavior.Default
                UserInputService.MouseIconEnabled = true
            end)
            ...
            pcall(function()
                if MenuInputLock.savedMouse ~= nil then
                    UserInputService.MouseBehavior = MenuInputLock.savedMouse
                end
                if MenuInputLock.savedIcon ~= nil then
                    UserInputService.MouseIconEnabled = MenuInputLock.savedIcon
                end
            end)
            MenuInputLock.savedMouse = nil
            MenuInputLock.savedIcon = nil
        end
    end
...
local crosshairHide = { lastHideState = nil, conns = {}, originals = {} }

local function saveOriginal(child)
    if crosshairHide.originals[child] then return end
    ...
    crosshairHide.originals[child] = state
end

local function applyCrosshair(child)
...
local function restoreCrosshair()
    for child, state in pairs(crosshairHide.originals) do
        if child and child.Parent then
            pcall(function()
                if child:IsA("Frame") then
    ...
    table.clear(crosshairHide.originals)
end

local function disconnectCrosshair()
    for _, conn in ipairs(crosshairHide.conns) do
...
local function setupCrosshair()
    ...
    if hideEnabled == crosshairHide.lastHideState then return end
    crosshairHide.lastHideState = hideEnabled
    disconnectCrosshair()
        ...
        if gui then
            ...
            if ch then
                for _, child in ipairs(ch:GetDescendants()) do
                    applyCrosshair(child)
                end
                table.insert(crosshairHide.conns, ch.DescendantAdded:Connect(function(child)
                    applyCrosshair(child)
                end))
            end
        end
...
getgenv().ValenokRestoreCrosshair = function()
    disconnectCrosshair()
    restoreCrosshair()
    crosshairHide.lastHideState = nil
end

Toggles.MiscHideCrosshair:OnChanged(setupCrosshair)
Toggles.MiscCenterDot:OnChanged(setupCrosshair)
task.spawn(setupCrosshair)
end)()

Shared.AmbienceState = {
    baseAmbient = nil,
    baseOutdoorAmbient = nil,
    baseTechnology = nil,
    baseBrightness = nil,
    SaturationCC = nil,
}
...
task.spawn(function()
    ...
    while Shared.AmbienceState.LoopRunning do
            ...
            if betterShadows ~= prevShadows then
                prevShadows = betterShadows
                if betterShadows then
                    if Shared.AmbienceState.baseTechnology == nil then
                        pcall(function() Shared.AmbienceState.baseTechnology = gethiddenproperty(Lighting, "Technology") end)
                    end
                    pcall(function() sethiddenproperty(Lighting, "Technology", Enum.Technology.ShadowMap) end)
                else
                    if Shared.AmbienceState.baseTechnology ~= nil then
                        pcall(function() sethiddenproperty(Lighting, "Technology", Shared.AmbienceState.baseTechnology) end)
                        Shared.AmbienceState.baseTechnology = nil
                    end
                end
            end

            if Toggles.LightingAmbient and Toggles.LightingAmbient.Value then
                if Shared.AmbienceState.baseAmbient == nil then Shared.AmbienceState.baseAmbient = Lighting.Ambient end
                Lighting.Ambient = getOptionColor("LightingAmbientColor", Color3.fromRGB(128, 128, 128))
            else
                if Shared.AmbienceState.baseAmbient ~= nil then
                    Lighting.Ambient = Shared.AmbienceState.baseAmbient
                    Shared.AmbienceState.baseAmbient = nil
                end
            end
            ...
            if anyLightingOn then
                ...
                if Shared.AmbienceState.baseBrightness == nil then Shared.AmbienceState.baseBrightness = Lighting.Brightness end
                Lighting.Brightness = lbright
            else
                if Shared.AmbienceState.baseBrightness ~= nil then
                    Lighting.Brightness = Shared.AmbienceState.baseBrightness
                    Shared.AmbienceState.baseBrightness = nil
                end
            end

            if Toggles.LightingGradient and Toggles.LightingGradient.Value then
                if Shared.AmbienceState.baseAmbient == nil then Shared.AmbienceState.baseAmbient = Lighting.Ambient end
                if Shared.AmbienceState.baseOutdoorAmbient == nil then Shared.AmbienceState.baseOutdoorAmbient = Lighting.OutdoorAmbient end
                ...
                if Shared.AmbienceState.baseOutdoorAmbient ~= nil then
                    Lighting.OutdoorAmbient = Shared.AmbienceState.baseOutdoorAmbient
                    Shared.AmbienceState.baseOutdoorAmbient = nil
                end
            end
...
RuntimePack._oldNamecall = nil
RuntimePack._oldNewindex = nil
RuntimePack._newindexInstalled = false

;(function()
local function silentTargetPos(targetPart)
...
local function applySilentHitParl(args)
    ...
    local tgtPlayer = Players:GetPlayerFromCharacter(tgtChar)
    local tgtHum = tgtChar and tgtChar:FindFirstChildOfClass("Humanoid")
    if not tgtPlayer or tgtPlayer == LocalPlayer then return args end
    if not tgtHum or tgtHum.Health <= 0 then return args end
    if hasShield(tgtChar) then return args end

    local teamCheck = RuntimePack.rageSilentActive and Toggles.RagebotTeamCheck or Toggles.SilentAimTeamCheck
    if not isEnemyFor(tgtPlayer, teamCheck) then return args end

    if RuntimePack.rageSilentActive then
        if not CombatScan.rageWanted() then return args end
    elseif RuntimePack.silentAimActive then
        if not CombatScan.silentWanted() then return args end
    else
        return args
    end

    local hitPos = silentTargetPos(tgt)
    if typeof(hitPos) ~= "Vector3" then return args end

    local cam = getCamera()
    if not cam then return args end
    ...
    if RuntimePack.rageSilentActive then
        ...
        if tgt == CombatScan.ragePart and hitPos == CombatScan.ragePoint then
            ...
            walls = getWallCount(camPos, hitPos, maxWalls, tgtChar)
        end

        canHit = walls <= maxWalls
    else
        walls = getWallCount(camPos, hitPos, 0, tgtChar)
        canHit = not (Toggles.SilentAimVisibleCheck
            and Toggles.SilentAimVisibleCheck.Value
            and walls > 0)
    end
...
restoreNamecallHook = function()
    ...
    pcall(function()
        ...
        if trueOld and not getgenv()._ValenokNamecallRestored then
            ...
            RuntimePack._oldNamecall = nil
        elseif RuntimePack._oldNamecall then
            hookmetamethod(game, "__namecall", RuntimePack._oldNamecall)
            RuntimePack._oldNamecall = nil
        end
    end)
end
...
pcall(function()
    ...
    RuntimePack._oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
        if not getgenv()._ValenokHooksActive then
            return RuntimePack._oldNamecall(self, ...)
        end
            ...
            if Toggles.VMOffsetEnable and Toggles.VMOffsetEnable.Value and self.Name ~= "HumanoidRootPart" then
                ...
                if isArms then
                    ...
                    if typeof(cf) == "CFrame" then
                        ...
                        return RuntimePack._oldNamecall(self, cf, select(2, ...))
                    end
                end
            end
            return RuntimePack._oldNamecall(self, ...)
        ...
        if method == "FireServer" or method == "FireUnreliable" then
            ...
            if name == "ControlTurn" then
                ...
                if Toggles.AntiAimPitchEnable and Toggles.AntiAimPitchEnable.Value then
                    ...
                    if pitchMode ~= "None" then
                        ...
                        return RuntimePack._oldNamecall(self, unpack(hookArgs))
                    end
                end
                return RuntimePack._oldNamecall(self, ...)
            end
            if name == "HitParl" then
                ...
                if not hitPart or not hitPart.Parent then
                    return RuntimePack._oldNamecall(self, unpack(args, 1, args.n))
                end
                ...
                if (Toggles.MiscHitSound and Toggles.MiscHitSound.Value)
                    ...
                    task.spawn(function()
                        pcall(function()
                            if Toggles.MiscHitSound and Toggles.MiscHitSound.Value then RuntimePack.PlayHitSound() end
                        end)
                        pcall(function()
                            if Toggles.MiscHitMarker and Toggles.MiscHitMarker.Value then RuntimePack.ShowHitMarker() end
                        end)
                    end)
                end
                return RuntimePack._oldNamecall(self, unpack(args, 1, args.n))
                            ...
                            if startPos and endPos then
                                if RuntimePack.silentActive then
                                    ...
                                    if silentTarget and silentTarget.Parent then
                                        local tp = silentTargetPos(silentTarget)
            ...
            if name == "ReplicateShot" then
                pcall(function()
                    if Toggles.PeekAssistEnable and Toggles.PeekAssistEnable.Value then
                        RuntimePack.peekAssistOnShot()
                    end
                end)

                local autoFireOn = Toggles.RagebotAutoFire and Toggles.RagebotAutoFire.Value
                local shouldInject = RuntimePack.rageSilentActive or RuntimePack.silentAimActive
                local rageFireBlocked = RuntimePack.rageSilentActive and autoFireOn

                if not rageFireBlocked
                    and not HitpartSilent.injecting
                    and shouldInject
                    and RuntimePack.canCombatFire()
                then
                    local silentTarget = getgenv().PSilentTarget
                    local silentPoint = getgenv().PSilentAimPoint
                    if silentTarget and silentTarget.Parent then
...
if not getgenv()._ValenokTrueNamecall then
    getgenv()._ValenokTrueNamecall = RuntimePack._oldNamecall
end
...
restoreNewindexHook = function()
    ...
    pcall(function()
        ...
        if trueOld and not getgenv()._ValenokNewindexRestored then
            ...
            RuntimePack._oldNewindex = nil
        elseif RuntimePack._oldNewindex then
            hookmetamethod(game, "__newindex", RuntimePack._oldNewindex)
            RuntimePack._oldNewindex = nil
        end
    end)
end
...
local function installSpoofHook()
    if RuntimePack._newindexInstalled then return end
    RuntimePack._newindexInstalled = true
    pcall(function()
        RuntimePack._oldNewindex = hookmetamethod(game, "__newindex", function(obj, prop, value)
        if not getgenv()._ValenokHooksActive then
            return RuntimePack._oldNewindex(obj, prop, value)
        end
        ...
        if not getgenv()._ValenokTrueNewindex then
            getgenv()._ValenokTrueNewindex = RuntimePack._oldNewindex
        end
...
if Toggles.MiscSpoofName then
    Toggles.MiscSpoofName:OnChanged(function(v)
        if v then installSpoofHook() end
    end)
    if Toggles.MiscSpoofName.Value then installSpoofHook() end
end
...
Shared.ensureFovCircles()
getgenv().ValenokFovCircles = { AimRuntime.AimFovCircle, AimRuntime.SilentFovCircle, AimRuntime.RageFovCircle, AimRuntime.SpreadCircle, AimRuntime.SpreadText }

_hitSoundObj = Instance.new("Sound")
_hitSoundObj.Parent = workspace

RuntimePack.restoreClientAmmoSafe = function()
    local t = InfAmmoState and InfAmmoState.table
    if not isClientAmmoTable or not isClientAmmoTable(t) then
        t = findAmmoInClient and findAmmoInClient() or nil
...
local function restoreWeaponMods()
    ...
    pcall(function()
        local Weapons = getWeaponsFolder()
        if not Weapons then return end
        for weaponName, original in pairs(rcsBase) do
            local weaponFolder = Weapons:FindFirstChild(weaponName)
        ...
        for weaponName, original in pairs(recoilBase) do
            local weaponFolder = Weapons:FindFirstChild(weaponName)
            local spread = weaponFolder and weaponFolder:FindFirstChild("Spread")
            local recoil = spread and spread:FindFirstChild("Recoil")
            if recoil and recoil:IsA("NumberValue") and type(original) == "number" and original > 0 then
    ...
    pcall(function()
        local Client = clientEnv()
        if Client and baseAccuracySd ~= nil then
            Client.accuracy_sd = baseAccuracySd
            baseAccuracySd = nil
        end
    end)
...
unloadValenok = function()
    ...
    if Shared.LegitBhopState and Shared.LegitBhopState.Conn then
        pcall(function() Shared.LegitBhopState.Conn:Disconnect() end)
        Shared.LegitBhopState.Conn = nil
    end
    RuntimePack.MoveLoop.clear()
    if HitMarkerState.HeartbeatConn then
        HitMarkerState.HeartbeatConn:Disconnect()
        HitMarkerState.HeartbeatConn = nil
    end
    ...
    if SC.State.GloveInstances then
        for glove in pairs(SC.State.GloveInstances) do
            pcall(function() if glove and glove.Parent then glove:Destroy() end end)
            SC.State.GloveInstances[glove] = nil
        end
    end
    if getgenv()._SCInvPushLeaveConn then
    ...
    restoreNamecallHook()
    restoreNewindexHook()
    pcall(nanGuard.restore)
    getgenv().PSilentTarget = nil
    getgenv().PSilentAimPoint = nil
    getgenv().IgnoreRaycastHook = false
    RuntimePack.silentActive = false
    RuntimePack.rageSilentActive = false
    RuntimePack.silentAimActive = false
    RuntimePack.silentAimTarget = nil
    RuntimePack.silentAimPoint = nil
    ...
    for _, c in ipairs({ AimRuntime.AimFovCircle, AimRuntime.SilentFovCircle, AimRuntime.RageFovCircle, AimRuntime.SpreadCircle, AimRuntime.SpreadText }) do
        pcall(function() c.Visible = false; c:Remove() end)
    end
    AimRuntime.AimFovCircle = nil
    AimRuntime.SilentFovCircle = nil
    ...
    table.clear(RuntimePack.ChamsVisCache)
    table.clear(RuntimePack.RayIgnoreMemo)
    table.clear(RuntimePack.CharIgnorePartsCache)
    table.clear(RuntimePack.IgnoreRootsCache)
    rayIgnoreCache.list = nil
    rayIgnoreCache.t = 0
    rayIgnoreCache.ignoreTeammates = nil
    table.clear(CacheData)
    table.clear(CacheExpiry)
    InfAmmoState.table = nil
    AntiAimState.HipHeight = nil
    AntiAimState.YawJitterFlip = false
    AntiAimState.YawJitterIndex = 0
    AntiAimState.YawJitterLastSwitch = 0
    LoopState.rageTarget = nil
    killAllRemote = nil
    baseAccuracySd = nil
    RuntimePack.NoSpreadClient = nil
    if RuntimePack.ModeCache then RuntimePack.ModeCache.t, RuntimePack.ModeCache.value = 0, false end
    if RuntimePack.TeamIgnoreCache then RuntimePack.TeamIgnoreCache.t, RuntimePack.TeamIgnoreCache.value = 0, false end
    ...
    pcall(function()
        ...
        if Shared.AmbienceState then
            ...
            if Shared.AmbienceState.baseTechnology ~= nil then
                pcall(function() sethiddenproperty(LightingSvc, "Technology", Shared.AmbienceState.baseTechnology) end)
            end
            if Shared.AmbienceState.baseAmbient ~= nil then
                LightingSvc.Ambient = Shared.AmbienceState.baseAmbient
            end
            if Shared.AmbienceState.baseOutdoorAmbient ~= nil then
                LightingSvc.OutdoorAmbient = Shared.AmbienceState.baseOutdoorAmbient
            end
            if Shared.AmbienceState.baseBrightness ~= nil then
                LightingSvc.Brightness = Shared.AmbienceState.baseBrightness
            end
    ...
    pcall(Shared.restoreSpeedHackOriginal)
    pcall(function() VirtualInputManager:SendKeyEvent(false, RuntimePack.MoveUtil.MOVE_KEY_CTRL, false, game) end)
    ...
    pcall(function()
        local hum = RuntimePack.MoveUtil.localHumanoid()
        if hum then hum.WalkSpeed = CONSTANTS.walkSpeed end
    end)
    pcall(Shared.restoreFlyPhysics)
...
local function watchWeaponChange(character)
    if not character then return end
...
;(function()
    if LocalPlayer.Character then
        task.spawn(watchWeaponChange, LocalPlayer.Character)
    end
    EspRuntime.Connections.WeaponCharAdded = LocalPlayer.CharacterAdded:Connect(watchWeaponChange)
    ...
    EspRuntime.Connections.PlayerRemoving = Players.PlayerRemoving:Connect(function(player)
        pcall(function()
            ...
            Shared.removePlayerChams(player)
            Shared.removeHighlight(player)
            invalidateEspCache(player)
            if player.Character then RuntimePack.invalidateCharIgnoreParts(player.Character) end
        end)
    end)
    ...
    EspRuntime.Connections.NoclipCharAdded = LocalPlayer.CharacterAdded:Connect(function(character)
        pcall(function()
            ...
            invalidateEspCache(LocalPlayer)
            RuntimePack.invalidateCharIgnoreParts(character)
        end)
    end)
...
local function updateRagebot()
    ...
    RuntimePack.rageSilentActive = Toggles.RagebotEnable
        and Toggles.RagebotEnable.Value
        and isAlive
        and keybindActive

    RuntimePack.silentActive = RuntimePack.rageSilentActive or RuntimePack.silentAimActive
    ...
    elseif RuntimePack.silentAimActive then
        LoopState.rageTarget = nil
        getgenv().PSilentTarget = RuntimePack.silentAimTarget
        getgenv().PSilentAimPoint = RuntimePack.silentAimPoint
    else
        LoopState.rageTarget = nil
        getgenv().PSilentTarget = nil
        getgenv().PSilentAimPoint = nil
...
local function updateSilentAim()
    ...
    if not RuntimePack.canCombatFire() then
        if not RuntimePack.rageSilentActive then
            RuntimePack.silentActive = false
            getgenv().PSilentTarget = nil
            getgenv().PSilentAimPoint = nil
        end
    ...
    if not CombatScan.silentWanted() then
        RuntimePack.silentActive = RuntimePack.rageSilentActive
    ...
    local target = CombatScan.silentPart
    local point = CombatScan.silentPoint
    ...
    if not targetPlayer
        ...
        or hasShield(targetChar)
        or not isEnemyFor(targetPlayer, Toggles.SilentAimTeamCheck)
    then
        RuntimePack.silentActive = RuntimePack.rageSilentActive
        if not RuntimePack.rageSilentActive then
            getgenv().PSilentTarget = nil
    ...
    RuntimePack.silentAimActive = true
    RuntimePack.silentAimTarget = target
    RuntimePack.silentAimPoint = typeof(point) == "Vector3" and point or target.Position
    RuntimePack.silentActive = true

    if not RuntimePack.rageSilentActive then
        getgenv().PSilentTarget = target
        getgenv().PSilentAimPoint = RuntimePack.silentAimPoint
    end
end

local function setDrawingHidden(drawing, hidden)
    if drawing == nil then return end
    pcall(function()
        drawing.Visible = not hidden
    end)
end

local function hideMenuOverlays(hidden)
    hidden = hidden == true

    for _, drawingSet in pairs(EspRuntime.Drawings) do
        if type(drawingSet) == "table" then
            for _, drawing in pairs(drawingSet) do
                if type(drawing) == "table" then
                    for _, obj in pairs(drawing) do
                        if type(obj) ~= "table" then
                            setDrawingHidden(obj, hidden)
                        end
                    end
                else
                    setDrawingHidden(drawing, hidden)
                end
            end
        end
    end
    for _, drawing in pairs(EspRuntime.ItemDrawings) do
        setDrawingHidden(drawing, hidden)
    end

    if AimRuntime then
        setDrawingHidden(AimRuntime.AimFovCircle, hidden)
        setDrawingHidden(AimRuntime.SilentFovCircle, hidden)
        setDrawingHidden(AimRuntime.RageFovCircle, hidden)
        setDrawingHidden(AimRuntime.SpreadCircle, hidden)
        setDrawingHidden(AimRuntime.SpreadText, hidden)
    end

    if Shared.CrosshairState then
        setDrawingHidden(Shared.CrosshairState.Circle, hidden)
        setDrawingHidden(Shared.CrosshairState.Outline, hidden)
        setDrawingHidden(Shared.CrosshairState.StateText, hidden)
    end

    for _, drawing in pairs(RageHitLog.Texts) do
        setDrawingHidden(drawing, hidden)
    end

    for _, drawing in pairs(HitMarkerState.OutlineLines) do
        setDrawingHidden(drawing, hidden)
    end
    for _, drawing in pairs(HitMarkerState.FillLines) do
        setDrawingHidden(drawing, hidden)
    end
end

local MenuOverlayHidden = false

local function runMainUpdate(stepDt)
    local now = tick()
    RuntimePack.updateBulletTracers(now)
    local myChar = LocalPlayer.Character
    local myHum = myChar and myChar:FindFirstChildOfClass("Humanoid")
    local isAlive = myHum and myHum.Health > 0 and myChar.Parent
    ...
    local silentOk, silentErr = pcall(updateSilentAim)
    if not silentOk then warn("[Valenok] Silent Aim:", silentErr) end
    ...
    local menuOpen = Library and Library.IsMenuVisible and Library:IsMenuVisible()
    if menuOpen ~= MenuOverlayHidden then
        MenuOverlayHidden = menuOpen
        hideMenuOverlays(menuOpen)
    end

    if not menuOpen then
        Shared.updateFovCircle()
    else
        hideMenuOverlays(true)
    end
    ...
    updateThirdPerson()
    -- ambience is now event-driven (GetPropertyChangedSignal), no polling needed
    -- call once per toggle change via UI callbacks only
    if isAlive then
        updateTriggerbot()
        updateAntiAim()
        updateNadePreview(stepDt)
        updatePeekAssist()
    end
...
task.spawn(function()
	Library:Notify({
		...
		Description = ScriptStatus.." "..ScriptVersion,
		Time = 3
	})

	task.wait(2)

	if RuntimePack.CbVersion ~= ScriptVersion then
		Library:Notify({
			Title = "Game Version",
			Description = "Script is not running on the latest CB version!" .. RuntimePack.CbVersion,
			Time = 3
		})
	else
		Library:Notify({
			Title = "Game Version",
			Description = "Script is up to date on the latest CB version " .. RuntimePack.CbVersion,
			Time = 3
		})
	end
end)
...
;(function()
    ...
    local function resyncConfig()
        if not (Toggles and Options) then return end
        pcall(function() applyNoRecoil(tv("GunModsNoRecoil")) end)
        pcall(function() applyNoSpread(tv("GunModsNoSpread")) end)
        pcall(function() applyInstaEquip(tv("GunModsInstaEquip")) end)
        pcall(function() applyInstaReload(tv("GunModsInstaReload")) end)
        pcall(function() updateRCS() end)
        pcall(function() updateFullAuto() end)
        if not tv("GunModsRapidFire") then pcall(restoreAllRapidFireRates) end
        if not tv("ExploitInfAmmo") then pcall(RuntimePack.restoreClientAmmoSafe) end
        pcall(function() updateNoScope() end)
        pcall(function() updateNoFlash() end)
        pcall(function() setupNoSmoke() end)
        pcall(function() applySkyboxChanger() end)
        pcall(function() updateSpeedHack() end)
        pcall(function() updateNoclip() end)
        pcall(function() updateFly() end)
        pcall(function() updateBhop() end)
        pcall(function() updateLegitBhop() end)
        pcall(function() updateAutoJump() end)
        pcall(function() updateAutoCrouch() end)
        pcall(function() updateFakeDuck() end)
        pcall(function() Shared.updateTpNoClip() end)
        pcall(function() Shared.applyFovChanger() end)
        pcall(function() Shared.applyRemoveUIElements() end)
    end

    SaveManager.LoadJSON = function(self, content, ...)
        local success, err = origLoadJSON(self, content, ...)
    ...
    -- Render Optimization: Move CombatScan to Heartbeat
    RunService.Heartbeat:Connect(function()
        CombatScan.refresh(tick())
    end)

    -- Memory Leak Fix: Cleanup dead players
    Players.PlayerRemoving:Connect(function(player)
        if EspRuntime then
            if EspRuntime.Drawings and EspRuntime.Drawings[player] then
                pcall(EspRuntime.RemoveDrawingValue, EspRuntime.Drawings[player])
                EspRuntime.Drawings[player] = nil
            end
            if EspRuntime.Chams and EspRuntime.Chams[player] then
                pcall(EspRuntime.RemoveDrawingValue, EspRuntime.Chams[player])
                EspRuntime.Chams[player] = nil
            end
            if EspRuntime.Highlights and EspRuntime.Highlights[player] then
                pcall(EspRuntime.RemoveDrawingValue, EspRuntime.Highlights[player])
                EspRuntime.Highlights[player] = nil
            end
        end
        if EspPlayerCache then
            EspPlayerCache[player] = nil
        end
        if CombatScan and CombatScan.byPlayer then
            CombatScan.byPlayer[player] = nil
        end
    end)
end)()
