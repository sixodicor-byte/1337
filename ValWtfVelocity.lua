...
-- ESP ScreenGui (replaces Drawing API)
local EspGui = Instance.new("ScreenGui")
EspGui.Name = "ValenokESP"
EspGui.ResetOnSpawn = false
EspGui.IgnoreGuiInset = true
EspGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
if getgenv().ValenokEspGui then
    pcall(function() getgenv().ValenokEspGui:Destroy() end)
end
getgenv().ValenokEspGui = EspGui
local _parentGui = gethui and gethui() or LocalPlayer:WaitForChild("PlayerGui")
EspGui.Parent = _parentGui
...
-- Bhop mode
MovementSections.Bhop:AddDropdown('BhopMode', {Values = { 'Default', 'Velocity' }, Default = 'Default', Text = 'Mode'})
...
AimRuntime.FovCircle = FovCircle
...
local function updateFovCircle()
    ...
    local FovCircle = AimRuntime.FovCircle
...
-- Bhop helpers
local BhopVelocityState = { lastSpeed = 0 }

local function updateBhop()
    if not Toggles.BhopEnable or not Toggles.BhopEnable.Value then
        BhopVelocityState.lastSpeed = 0
        return
    end
    ...
    local mode = Options.BhopMode and Options.BhopMode.Value or "Default"
    ...
    if mode == "Velocity" then
        ...
        if onGround then
            humanoid.Jump = true
            -- Preserve speed from previous frame
            local preservedSpeed = BhopVelocityState.lastSpeed
            if preservedSpeed > 16 then
                local currentVel = rootPart.AssemblyLinearVelocity
                local horizMag = math.sqrt(currentVel.X^2 + currentVel.Z^2)
                if horizMag > 0 then
                    local scale = preservedSpeed / horizMag
                    rootPart.AssemblyLinearVelocity = Vector3.new(currentVel.X * scale, currentVel.Y, currentVel.Z * scale)
                end
            end
        end

        -- Strafe acceleration: add velocity in look direction
        local camLook = Camera.CFrame.LookVector
        ...
        local currentVel = rootPart.AssemblyLinearVelocity
        local accel = 2 * multiplier
        local maxSpeed = 16 * multiplier * 3

        local newVelX = currentVel.X + mx * accel
        local newVelZ = currentVel.Z + mz * accel

        local horizMag = math.sqrt(newVelX^2 + newVelZ^2)
        if horizMag > maxSpeed then
            local scale = maxSpeed / horizMag
            newVelX = newVelX * scale
            newVelZ = newVelZ * scale
            horizMag = maxSpeed
        end

        rootPart.AssemblyLinearVelocity = Vector3.new(newVelX, currentVel.Y, newVelZ)
        BhopVelocityState.lastSpeed = horizMag
    else
        -- Default method
...
ConfigSection:AddButton('Unload', function()
    ...
    -- Cleanup FOV circle
    if AimRuntime.FovCircle then
        pcall(function() AimRuntime.FovCircle:Destroy() end)
        AimRuntime.FovCircle = nil
    end
    ...
    -- Cleanup ESP (destroy EspGui removes all children)
    if EspGui then
...
-- Create square (Frame with border)
local function createSquare(Thickness, Color)
    ...
    Stroke.Transparency = (Thickness == 0) and 1 or 0
    Stroke.Parent = Frame
    Frame.Visible = false
    Frame.Parent = EspGui
    return Frame
...
-- Create line (Frame, used for grenade prediction)
...
local function hideDrawingSet(DrawingSet, ResetRect)
    ...
    if DrawingSet.Box then DrawingSet.Box.Visible = false end
    if DrawingSet.Name then DrawingSet.Name.Visible = false end
    if DrawingSet.Weapon then DrawingSet.Weapon.Visible = false end
    if DrawingSet.HealthBarOutline then DrawingSet.HealthBarOutline.Visible = false end
    if DrawingSet.HealthBarFill then DrawingSet.HealthBarFill.Visible = false end
    if DrawingSet.HealthText then DrawingSet.HealthText.Visible = false end
...
local function removeDrawingSet(Player)
    ...
    for _, Item in DrawingSet do
        if type(Item) == "userdata" and Item.Destroy then
            pcall(function()
                Item:Destroy()
            end)
        end
...
local function getDrawingSet(Player)
    ...
    DrawingSet = {
        ...
        HealthBarFill = createSquare(0, Color3.fromRGB(0, 255, 0)),
...
local function updatePlayerEsp(Player)
    ...
    DrawingSet.Box.Position = UDim2.fromOffset(Left, Top)
    DrawingSet.Box.Size = UDim2.fromOffset(Width, Height)
    local boxStroke = DrawingSet.Box:FindFirstChildOfClass("UIStroke")
    if boxStroke then boxStroke.Color = BoxColor end
    DrawingSet.Box.Visible = ShowBox

    DrawingSet.Name.Text = Player.Name
    DrawingSet.Name.Position = UDim2.fromOffset(CenterX, Top - 15)
    DrawingSet.Name.TextColor3 = NameColor
    ...
    DrawingSet.Weapon.Position = UDim2.fromOffset(CenterX, Bottom + 5)
    DrawingSet.Weapon.TextColor3 = WeaponColor
    ...
    if ShowHealthBar and Humanoid then
        ...
        DrawingSet.HealthBarOutline.Position = UDim2.fromOffset(barX, barY)
        DrawingSet.HealthBarOutline.Size = UDim2.fromOffset(barWidth, barHeight)
        local outlineStroke = DrawingSet.HealthBarOutline:FindFirstChildOfClass("UIStroke")
        if outlineStroke then outlineStroke.Color = Color3.fromRGB(0, 0, 0) end
        DrawingSet.HealthBarOutline.Visible = true

        local fillHeight = barHeight * hpPercent
        local fillY = barY + (barHeight - fillHeight)
        DrawingSet.HealthBarFill.Position = UDim2.fromOffset(barX + 1, fillY)
        DrawingSet.HealthBarFill.Size = UDim2.fromOffset(barWidth - 2, fillHeight)
        DrawingSet.HealthBarFill.BackgroundColor3 = getOptionColor("ESPHealthBarColor", Color3.fromRGB(0, 255, 0))
        DrawingSet.HealthBarFill.BackgroundTransparency = 0
        DrawingSet.HealthBarFill.BorderSizePixel = 0
        DrawingSet.HealthBarFill.Visible = true

        local hp = math.floor(Humanoid.Health)
        if hp < 100 then
            DrawingSet.HealthText.Text = tostring(hp)
            DrawingSet.HealthText.Position = UDim2.fromOffset(barX - 15, barY)
            DrawingSet.HealthText.TextColor3 = Color3.fromRGB(255, 255, 255)
...
local function updateGrenadePrediction()
    ...
    for i = 1, #points - 1 do
        ...
        if screen1.Z > 0 and screen2.Z > 0 then
            ...
            line.Position = UDim2.fromOffset(screen1.X, screen1.Y)
            line.Rotation = angle
            line.Visible = true
        end
    end
