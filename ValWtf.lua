    ...
    local updateNoScope
    local updateNoFlash
    local updateServerPos
    local updateLegitBhop
    ...
    -- Server pos
    RageSections.Exploit:AddToggle('ExploitServerPos', {Text = 'Desync', Default = false, Callback = function() updateServerPos() end})


    -- Server pos keybind
    RageSections.Exploit:AddLabel('Keybind'):AddKeyPicker('ExploitServerPosKeybind', {Default = 'None', Mode = 'Toggle', Text = 'Server pos'})
    ...
    do
        ...
        local function makeHMLine()
            local l = Drawing.new("Line")
            l.Thickness = 2
            l.Visible = false
            return l
        end
        for i = 1, 4 do _hmLines[i] = makeHMLine() end
        ShowHitMarker = function()
            ...
            local gap, len = 5, 10
            local col = Options.MiscHitMarkerColor and Options.MiscHitMarkerColor.Value or Color3.fromRGB(255, 255, 255)
            local segs = {
            ...
            for i, seg in ipairs(segs) do
                _hmLines[i].From = seg[1]
                _hmLines[i].To = seg[2]
                _hmLines[i].Color = col
                _hmLines[i].Visible = true
            end
            task.delay(0.15, function()
                for _, l in ipairs(_hmLines) do l.Visible = false end
                _hmActive = false
            end)
        end
    end
    ...
    local CachedRayIgnore = nil
    local LastRayIgnoreRefresh = 0
    local function getCachedRayIgnore()
        local now = tick()
        if not CachedRayIgnore or not CachedRayIgnore.Parent or now - LastRayIgnoreRefresh > 1.5 then
            CachedRayIgnore = Workspace:FindFirstChild("Ray_Ignore")
            LastRayIgnoreRefresh = now
        end
        return CachedRayIgnore
    end
    ...
    local FovCircleWasHidden = false
    local function updateFovCircle()
        ...
        if not ShowAimFov and not ShowRageFov then
            if not FovCircleWasHidden then
                for _, Line in ipairs(AimRuntime.FovLines) do
                    Line.Visible = false
                end
                FovCircleWasHidden = true
            end
            return
        end
        FovCircleWasHidden = false
    ...
    local function updateAimBot()
        ...
        local now = tick()
        if not isPartTargetable(TargetPart, ScreenCenter, FovRadius) or (now - AimLastTargetScan >= (1/30)) then
    ...
    local function updateRagebot()
        ...
        if not ragebotEnabled or not keyActive then
            ...
            RagebotState.CachedTarget = nil
            return
        end

        local now = tick()
        local targetPart = RagebotState.CachedTarget
        if not targetPart or not targetPart.Parent or (now - RagebotState.LastTargetScan >= (1/30)) then
            targetPart = getRagebotTarget()
            RagebotState.CachedTarget = targetPart
            RagebotState.LastTargetScan = now
        end
    ...
    updateLegitBhop = function()
        ...
        LegitBhopState.Conn = RunService.RenderStepped:Connect(function()
            ...
            local multiplier = 1 + (math.min(LegitBhopState.JumpCount, 10) / 10) * (maxMult - 1)
            humanoid.WalkSpeed = LegitBhopState.DefaultSpeed * multiplier
        end)
    end
    ...
    local function unloadValenok()
        ...
        -- Cleanup ServerPos (Desync)
        if EspRuntime.CleanupServerPos then EspRuntime.CleanupServerPos() end
        if EspRuntime.RemoveServerPosHook then EspRuntime.RemoveServerPosHook() end
        if EspRuntime.ServerPosText then pcall(function() EspRuntime.ServerPosText:Remove() end) end
        EspRuntime.ServerPosText = nil
    ...
    local function updateTriggerbot()
        ...
        if Toggles.TriggerbotMagnet and Toggles.TriggerbotMagnet.Value then
            ...
            if now - (TriggerbotState.LastMagnetScan or 0) >= (1/30) then
                ...
                TriggerbotState.CachedMagnetTarget = magnetTarget
            else
                magnetTarget = TriggerbotState.CachedMagnetTarget
            end
    ...
    local function updateAmbience()
        ...
        local anyEnabled = (Toggles.AmbienceCustomTime and Toggles.AmbienceCustomTime.Value)
            ...
            or (Options.AmbienceBrightness and Options.AmbienceBrightness.Value ~= 0)
        ...
        if Options.AmbienceBrightness then
            Lighting.Brightness = 2 + Options.AmbienceBrightness.Value
        end
    end
    ...
    -- Server pos (raknet desync)
    local ServerPosText = createText(13)
    EspRuntime.ServerPosText = ServerPosText
    local ServerPosFrozen = nil
    local ServerPosHookAdded = false
    local ServerPosActive = false

    local function serverPosShouldRun()
        local toggleOn = Toggles.ExploitServerPos and Toggles.ExploitServerPos.Value
        local keyOn = Options.ExploitServerPosKeybind and Options.ExploitServerPosKeybind:GetState()
        return toggleOn and keyOn
    end

    local function serverPosDesyncHook(packet)
        if not ServerPosActive then return end
        pcall(function()
            if not packet then return end
            if packet.PacketId ~= 0x1B then return end
            local buf = packet.AsBuffer
            if not buf then return end
            if not buffer or not buffer.len or not buffer.writeu32 then return end
            if buffer.len(buf) < 5 then return end
            if not packet.SetData then return end
            buffer.writeu32(buf, 1, 0xFFFFFFFF)
            packet:SetData(buf)
        end)
    end

    -- Register the raknet hook ONCE; toggling is done via ServerPosActive flag
    -- to avoid crashes from rapid add/remove of the same hook.
    if raknet then
        local ok = pcall(raknet.add_send_hook, serverPosDesyncHook)
        if ok then ServerPosHookAdded = true end
    end

    local function cleanupServerPos()
        ServerPosActive = false
        ServerPosFrozen = nil
        if ServerPosText then ServerPosText.Visible = false end
    end
    EspRuntime.CleanupServerPos = cleanupServerPos

    EspRuntime.RemoveServerPosHook = function()
        if ServerPosHookAdded and raknet then
            pcall(function() raknet.remove_send_hook(serverPosDesyncHook) end)
        end
        ServerPosHookAdded = false
        ServerPosActive = false
        ServerPosFrozen = nil
    end

    updateServerPos = function()
        if not serverPosShouldRun() then
            cleanupServerPos()
        end
    end

    EspRuntime.Connections.ServerPosRenderStepped = RunService.RenderStepped:Connect(function()
        local run = serverPosShouldRun()

        if run and not ServerPosActive then
            local character = LocalPlayer.Character
            local hrp = character and character:FindFirstChild("HumanoidRootPart")
            if not hrp then
                ServerPosText.Visible = false
                return
            end
            ServerPosFrozen = hrp.Position
            ServerPosActive = true
        elseif not run and ServerPosActive then
            ServerPosActive = false
            ServerPosFrozen = nil
            local character = LocalPlayer.Character
            local hrp = character and character:FindFirstChild("HumanoidRootPart")
        ...
        if not run or not ServerPosFrozen then
            ServerPosText.Visible = false
            return
        end
        ...
        local point = cam:WorldToViewportPoint(ServerPosFrozen)
        if point.Z > 0 then
            ServerPosText.Text = "SERVER POS"
            ServerPosText.Color = getOptionColor("ESPNameColor", Color3.fromRGB(255, 255, 255))
            ServerPosText.Position = Vector2.new(point.X, point.Y)
            ServerPosText.Visible = true
        else
            ServerPosText.Visible = false
        end
    end)
